...
| `EVIDENCE_RECLASSIFY` | 결정론 문지기가 채우지 못한 장의 근거를 AI가 한 번 재판정하고, 프로그램 검증을 통과한 정확 인용만 후보에 보탠다. **현재 출시 Blueprint에서 `"1"`로 켜져 있다**(2026-09-16 사용자 결정 — 빈 장을 채우는 자동 경로가 운영에서 하나도 돌지 않던 실측). | 키를 지우고 재배포 |
...
| `PARALLEL_COLLECT` | 본조사의 경쟁사 비교와 뉴스 검색 스냅샷을 동시에 돌린다. 보고서 결과·캐시 열쇠·실패 의미는 그대로고 순서만 바뀐다. 비교가 멈추는 실행에서는 이미 떠난 뉴스 검색 호출을 되돌릴 수 없다. **현재 출시 Blueprint에서 `"1"`로 켜져 있다.** | 키를 지우고 재배포 |
...
### 기본이 켜짐이고 "0"이면 꺼지는 스위치

`GROUNDING_REWRITE`는 위 표와 방향이 반대다. **선언하지 않아도 기본이 켜짐**이고,
값을 정확히 `"0"`으로 적어야 꺼진다. 근거 결속 검사에서 탈락한 «확인» 문장을 한
번 묶어 고쳐 써서 살리는 기능이며, 켜져 있을 때와 껐을 때의 결과를 운영에서
바로 비교 실측하기 위해 기본을 켬으로 둔다. **현재 출시 Blueprint에서 `"1"`로
명시돼 있다.**

⚠️ Render 대시보드에 저장된 환경변수 값은 이 `render.yaml`(Blueprint)과 **자동으로
동기화되지 않는다.** 값을 바꾸려면(예: `GROUNDING_REWRITE`를 `"0"`으로) 대시보드에서
직접 바꿔야 한다.
