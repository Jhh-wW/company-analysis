...
DART 공시를 중심으로 조사하고, 공식 발표와 신뢰할 수 있는 언론 보도로 최신 사업 근거를 보강합니다. 회사를 찾을 때는 DART 전체 기업 목록을 그대로 대조하므로, 목록에 없는 회사를 만들어 내지 않습니다. 뉴스도 실제 본문·회사 일치·날짜를 검증하며, 확인되지 않은 내용을 자유롭게 만들지 않습니다. 웹 · PDF · Notion은 같은 검증 결과를 사용합니다.
...
docs/               보고서 정본 기준 · 아키텍처 · ADR · 실행 검증 기록
...
네 묶음은 각각 따로 실행합니다. `app/src`와 `analysis_engine/src`를 한 세션에 모으면 같은 이름의 시험 파일이 겹쳐 수집이 중단됩니다. 네 묶음이 모두 통과한 변경만 `main`에 합칩니다([배포 절차](deploy/README.md)). 시험 건수처럼 변하는 숫자는 문서에 적지 않습니다 — 위 배지와 이 명령으로 직접 확인합니다.
...
같은 이름의 다른 회사를 잘못 분석하지 않기 위해서입니다. 후보는 DART 전체 기업 목록에서
이름과 주소로 찾습니다. 띄어쓰기·약칭·영문 표기 차이는 규칙으로 먼저 흡수하고, 규칙이
후보를 하나도 못 찾을 때만 AI에게 **정식 법인명이 무엇일지 묻습니다.** AI가 준 이름으로도
같은 목록을 다시 찾을 뿐이라, AI가 지어낸 회사는 후보가 되지 않습니다.
...
검수 AI의 「참」을 그대로 믿지 않고, **그 문장이 댄 근거를 실제 원문에 다시 맞춰 보는
기계 검사**를 한 겹 더 겁니다. 근거를 못 대면 검수가 통과시켰어도 문장이 빠집니다.
...
| 3 | 핵심 제품·서비스와 포트폴리오 역할 | 8 | 인재상과 일하는 방식 |
| 4 | 3개년 주요 변화와 실행 `#과거` | 9 | 회사가 밝힌 차별점 |
| | | 부록 | 출처와 검증 상태 |

1~9장이 모두 필수입니다. 9장은 다른 회사와 숫자를 맞대는 비교표가 아니라, **회사가 공식
자료에서 스스로 밝힌 차별점**과 그 한계를 쓰는 장입니다. 같은 조건으로 견줄 수 있는 자료가
있을 때만 동종업계 비교를 뒤에 덧붙입니다.
근거가 얇으면 그 장을 한 번 더 채워보고, 그래도 기준에 못 미치면 9장만 뺀 보고서를 내는
대신 **보고서를 내지 않습니다.** 이때는 요금을 청구하지 않습니다.

## 기능

- **회사 찾기** — DART 전체 기업 목록을 색인해 이름과 주소로 후보를 찾습니다. 띄어쓰기·약칭·
  영문 표기 차이는 규칙이 흡수하고, 규칙이 하나도 못 찾을 때만 AI가 정식 법인명을 제안합니다.
  같은 점수 후보가 화면 상한을 넘어 순서가 안 갈릴 때만 AI가 순서를 다시 매깁니다. AI는 후보를
  **제안**만 하고 확정하지 않으며, 응답이 이상하면 규칙 결과를 그대로 씁니다.
...
- **문장 단위 검증 관문** — 검수 AI의 판정을 그대로 받지 않고 기계 검사를 겹칩니다. 인용한
  원문에 그 숫자·항목·기간이 실제로 있는지 재계산하고, `'26년`처럼 줄여 쓴 연도를 같은 해로
  읽습니다. 「N년 연속」은 값이 그만큼 있어야 통과하고, 임원 직함은 인용 원문의 기준일로
  재직 여부를 봅니다. 조직문화 장에 섞여 든 회계 규정 문장, 과거·현재 사실을 미래 계획으로
  올린 문장, 근거 없는 대응 칸은 각각 따로 걸러 냅니다.
- **장 간 중복 제거와 요약 결속** — 표현만 바꾼 같은 사실을 장 사이에서 지우고, 핵심 요약은
  본문에서 이미 검증된 문장을 글자 그대로 재사용합니다. 요약이 본문에 없는 말을 만들 수 없습니다.
...
**AI에게는 이름을 묻고, 회사는 목록으로 정한다** — 사람들이 흔히 부르는 이름은 공시에 등록된
상호와 글자가 하나도 겹치지 않는 경우가 있어, 글자를 맞춰 보는 규칙만으로는 찾지 못합니다.
그래서 AI에게 정식 법인명을 묻되, 받은 이름으로 **DART 목록을 다시 찾는 데만** 씁니다.
회사별 사전은 만들지 않았습니다. 한 회사를 위한 예외는 그 회사만 좋아지기 때문입니다.

**품질은 시험만으로 못 잰다** — 자동 시험은 규칙이 지켜지는지만 봅니다. 그래서 실제 회사를
유료로 돌려 보고서를 만든 뒤, 만든 쪽과 별개인 검수를 붙이는 절차를 따로 둡니다. 소스가 실행
중에 바뀌지 않았는지 먼저 확인하고, 실행마다 비용을 정산한 뒤 결과를 읽습니다.

**뉴스는 별도의 최신 근거 조사다** — 공시가 충분해도 최근 보도를 확인합니다. 제목만 보고 첫 문장을 붙이지 않고 회사·사업 관련성과 원문을 먼저 검증합니다. 확인된 숫자는 출처·날짜와 함께 본문에 쓰고 기사 목록도 제공합니다. 자료 부족과 검색·본문 확인 장애는 구분합니다. 상세 정책은 [뉴스 조사 계약](docs/출력물 기준/90_공통_규칙/자료_수집과_근거_뉴스.md)을 따릅니다.

**확인된 내용과 확인 한계를 함께 제공한다** — 회사 신원·출처 결속·출고 안전 검사가 깨진 결과는 막습니다. 반면 일부 뉴스 접근 실패나 자료 희소를 숨기거나 확인된 공식 사실까지 폐기하지 않습니다. 보고서에는 장마다 무엇까지 확인했고 무엇을 확인하지 못했는지를 함께 적습니다.
...
- **DART에 사업보고서나 감사보고서를 내는 법인만 대상입니다.** 이 두 가지가 재무 근거의
  뿌리라, 다른 양식으로만 공시하는 법인은 지금 구조로 분석할 수 없습니다.
- **자료가 적은 회사는 보고서가 아예 안 나올 수 있습니다.** 운영 설정이 `FULL`이라 부족한
  장을 한 번 보충해 보고, 그래도 기준에 못 미치면 부분 보고서 대신 전체를 막습니다. 규모가
  작은 회사는 신뢰할 수 있는 언론 기준을 통과하는 보도가 드물어 뉴스 근거도 거의 붙지 않습니다.
- **검증 관문의 세기는 계속 조정 중입니다.** 자동 시험은 규칙이 지켜지는지만 보고, 문장이
  실제로 읽을 만한지는 실제 회사로 돌려 봐야 압니다. 조이면 통과가 줄고 풀면 근거 없는 문장이
  늘어나는 맞바꿈이라, 실행 검수 결과를 보고 한 번에 한 겹씩 고치고 있습니다.
- **아무나 쓸 수 있는 서비스가 아닙니다.** `BETA_ADMIN_ONLY=1`이라 관리자와 초대 명단에
  있는 회원, 그리고 초대 링크로 들어온 사람만 조사를 시작할 수 있습니다.
- **저장소는 커밋 자동 배포를 선언합니다.** `render.yaml`의 `autoDeployTrigger: commit`이 실제
  Render 서비스에도 적용돼 있으면 main 푸시가 배포를 시작할 수 있습니다. 대시보드 설정과 배포된
  커밋을 따로 확인해야 하며, 푸시 완료만으로 운영 반영을 확정하지 않습니다.
...
- [검증 기록](docs/reviews/) — 실제 회사로 돌려 본 결과와 거기서 고친 것들
