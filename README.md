...
<!-- D-LIC 결정 뒤 root가 라이선스 배지를 여기 추가 -->
...
| 4 | 3개년 주요 변화와 실행 `#과거` | 9 | 동종업계 비교 *(조건부)* |
| | | 부록 | 출처와 검증 상태 |

1~8장은 필수입니다. 9장은 **양사 공식 원문을 같은 기준으로 비교할 수 있을 때만** 붙이고,
조건이 안 맞으면 억지로 비교하지 않고 9장을 뺀 `PARTIAL` 등급으로 표시합니다.
...
만들고 있는 기능(아직 화면 없음): 관리자가 만드는 초대 링크(QR) 열람 · 링크별 이용 한도(하루·누적) ·
관리자 명단과 회원별 한도.
