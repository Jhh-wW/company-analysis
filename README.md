...
DART 공시를 중심으로 조사하고, 공식 발표와 신뢰할 수 있는 언론 보도로 최신 사업 근거를 보강합니다. 뉴스도 실제 본문·회사 일치·날짜를 검증하며, 확인되지 않은 내용을 자유롭게 만들지 않습니다. 웹 · PDF · Notion은 같은 검증 결과를 사용합니다.
...
**확인된 내용과 확인 한계를 함께 제공한다** — 회사 신원·출처 결속·출고 안전 검사가 깨진 결과는 막습니다. 반면 일부 뉴스 접근 실패나 자료 희소를 숨기거나 확인된 공식 사실까지 폐기하지 않습니다. 출시 모드별 필수 검사는 그대로 적용하고, 부분 결과와 조사 한계를 명시합니다.
...
- **저장소는 커밋 자동 배포를 선언합니다.** `render.yaml`의 `autoDeployTrigger: commit`이
  실제 Render 서비스에도 적용돼 있으면 main 푸시가 배포를 시작할 수 있습니다. 대시보드 설정과
  배포된 커밋을 별도로 확인해야 하며, 푸시 완료만으로 운영 반영을 확정하지 않습니다.
