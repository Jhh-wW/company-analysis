...
_COMMIT_ENV_NAMES: tuple[str, ...] = ("RENDER_GIT_COMMIT", "APP_GIT_COMMIT")

#: 커밋을 짧게 자른다. 사람이 대시보드·git log 와 눈으로 맞추는 데 7자면 충분하고,
#: 전체 40자를 그대로 흘리지 않는다.
_COMMIT_SHORT_LEN: int = 7

#: 받아들일 최대 길이 — git SHA-1 전체가 40자다. 이보다 길면 커밋이 아니다.
_COMMIT_FULL_LEN: int = 40
...
def _deployed_commit() -> str:
    ...
    for name in _COMMIT_ENV_NAMES:
        ...
        if not raw or len(raw) > _COMMIT_FULL_LEN:
            continue
        if all(letter in "0123456789abcdef" for letter in raw):
            return raw[:_COMMIT_SHORT_LEN]
