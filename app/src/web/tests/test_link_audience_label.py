"""링크 표시 이름과 회사 고유번호 없는 보고서의 연결 화면을 확인한다.
...
★ 이 시험이 지키는 것 두 가지
  ...
  ② **회사 고유번호 없이도 연결한다.** 관리자가 선택한 보고서는 회사명이나
     고유번호 대조 없이 연결할 수 있고, 기존 연결 정보도 화면에 표시한다.
"""
...
def test_report_without_company_id_remains_editable(admin: TestClient):
...
def test_고유번호가_있으면_경고를_보이지_않는다(admin: TestClient):
    """고유번호가 있는 보고서에도 불필요한 연결 경고를 표시하지 않는다."""
