...
def test_링크를_발급하면_저장된다(admin: TestClient):
    ...
    assert (link.company, link.job, link.note) == ("카카오", "", "하반기 공채")
...
def test_기존_링크에도_보고서를_연결하고_다시_해제할수있다(admin: TestClient):
    ...
    attached = admin.post(
        "/admin/link/report",
        data={"key": key, "report_reference": f"/result/{report_id}"},
        follow_redirects=False,
    )
...
def test_기존_링크에도_없는_보고서를_연결할수없다(admin: TestClient):
    ...
    response = admin.post(
        "/admin/link/report",
        data={"key": key, "report_reference": "a" * 32},
        follow_redirects=False,
    )
...
def test_다른회사의_보고서는_새링크와_기존링크에_연결할수없다(
    admin: TestClient,
):
    ...
    existing_link = admin.post(
        "/admin/link/report",
        data={"key": key, "report_reference": report_id},
        follow_redirects=False,
    )

    assert new_link.status_code == existing_link.status_code == 400
    assert "링크 회사와 보고서 회사가 다릅니다" in new_link.text
    assert "링크 회사와 보고서 회사가 다릅니다" in existing_link.text
    with storage_db.connect() as conn:
        assert share_store.load(conn, key).report_id == ""
...
def test_링크철회를_확인할수없으면_성공으로_리디렉션하지않는다(
    ...
    if failure == "exception":
        ...
        monkeypatch.setattr(share_store, "delete", broken_delete)
    else:
        monkeypatch.setattr(share_store, "delete", lambda *_args, **_kwargs: False)

    response = admin.post(
        "/admin/link/delete", data={"key": key}, follow_redirects=False
    )
...
def test_회사링크를_닫아도_이미전달된_독립결과주소는_60일정책을_따른다(
    ...
    detail_before_close = admin.get(f"/admin/link/{key}")
...
def test_관리자_첫화면도_예상비용_차단기준이라고_정직하게_안내한다(
    admin: TestClient,
):
    text = admin.get("/admin").text

    assert "호출 전 예상비용 차단 기준" in text
    assert "최악의 하루 지출" not in text
...
def test_주소와_QR을_보여준다(admin: TestClient, monkeypatch):
...
def test_설정된_canonical_origin은_악성_Host와_forwarded_host에_안_바뀐다(
    admin: TestClient, monkeypatch
):
    monkeypatch.setenv("SHARE_PUBLIC_BASE_URL", "https://demo.example")
    created = admin.post(
    ...
    assert response.status_code == 200
    assert f"https://demo.example/k/{key}" in response.text
...
def test_정본공개주소가_없거나_잘못되면_악성Host로_QR을_만들지않는다(
    ...
    response = admin.get(f"/admin/link/{key}", headers={"Host": "evil.example"})

    assert response.status_code == 200
    assert "evil.example" not in response.text
    assert "<svg" not in response.text
    assert f"/k/{key}" in response.text
    assert "검증된 공개 서비스 주소가 없어" in response.text
...
def test_localhost_주소면_경고한다(admin: TestClient):
    ...
    text = admin.get(
        f"/admin/link/{key}", headers={"Host": "127.0.0.1:8000"}
    ).text
