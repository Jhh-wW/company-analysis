...
def _조사시작(client: TestClient, 회사: str = "우리엔"):
    return client.post("/run", data=_폼(회사), follow_redirects=False)
...
def _열쇠로_들어온다(client: TestClient) -> None:
    ...
    with storage_db.connect() as conn:
        share_store.save(
            conn, key=_열쇠, company="카카오", job="마케팅",
...
def _예산을_다_쓴다(monkeypatch, 통장: str = _열쇠,
    ...
    monkeypatch.setattr(main, "_LINK_SPEND", 다_쓴)
...
def test_날이_바뀌면_예산이_되살아난다(client: TestClient, monkeypatch):
    ...
    monkeypatch.setattr(main, "_LINK_SPEND", 다_쓴_장부)
...
def test_서로_다른_로그인_사용자_다섯명까지_자리를_잡는다():
    ...
    assert all(자리)
    assert main._reserve_run_slot(
        share_tracks.Track.MEMBER, "user:sixth@example.com"
    ) is None

    for bucket_id in 자리:
        main._release_run_slot(bucket_id or "")
...
def test_같은_초대링크는_세명까지_자리를_잡는다():
    자리 = [
        main._reserve_run_slot(share_tracks.Track.LINK, _열쇠)
        for _ in range(MAX_CONCURRENT_PER_LINK)
    ]

    assert all(자리)
    assert main._reserve_run_slot(share_tracks.Track.LINK, _열쇠) is None
...
def test_예산으로_거절당해도_횟수는_안_깎인다(client: TestClient, monkeypatch):
    ...
    assert main._RATE_HISTORY.starts == {}
...
def test_끝난_지_오래된_조사는_치운다():
    ...
    main._sweep_jobs(100.0 + 3601)

    assert "old1" not in main._JOBS
...
def test_돌고_있는_조사는_안_치운다():
    ...
    main._sweep_jobs(999999.0)

    assert "run1" in main._JOBS
