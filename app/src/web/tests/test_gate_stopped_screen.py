...
def _member_cookies(email: str = "member@example.com"):
...
def test_자동검사중단_5경로_어디에도_빈_href_버튼이_없다(gate_stopped, monkeypatch):
    report = gate_stopped
    job_id = f"gate-dead-end-{uuid.uuid4().hex}"
...
def test_자동검사중단_화면은_막은_사유와_문의번호를_보여준다(gate_stopped):
    report = gate_stopped
    job_id = f"gate-reasons-{uuid.uuid4().hex}"
...
def test_일시장애_화면은_다시_시도_아이콘과_안내를_그대로_쓴다():
    ...
    assert "잠시 후 같은 LINK로 다시 시도해 주세요." in html
...
def test_PDF를_못_만든_화면에도_문의_번호가_있다(monkeypatch):
    ...
    monkeypatch.setattr(job_runtime, "_load_saved_report", lambda _j: report)
    monkeypatch.setattr(job_runtime, "_link_expired", lambda _r: False)
    monkeypatch.setattr(reports_router, "_release_state", broken)

    with TestClient(app) as client:
        response = client.get(
            f"/download/pdf/pdf-fail-{uuid.uuid4().hex}", follow_redirects=False
        )

    assert response.status_code == 503
    assert "PDF 보고서를 잠시 만들 수 없습니다" in response.text
    assert response.headers["X-Request-ID"] in response.text
    assert "시험용 내부 오류" not in response.text
...
def test_자동검사중단_화면은_예외메시지_원문을_그리지_않는다(monkeypatch):
    ...
    monkeypatch.setattr(job_runtime, "_link_expired", lambda _report: False)
    monkeypatch.setattr(reports_router, "_release_state", stopped)
    job_id = f"gate-no-raw-{uuid.uuid4().hex}"
    cookies, _csrf = _admin_cookies()
...
def test_회원과_링크손님만_중단화면에서_오류신고로_갈_수_있다(gate_stopped):
    ...
    with TestClient(app) as client:
        ...
        anonymous = client.get(f"/result/{job_id}")
    ...
    # 완전 익명은 POST /feedback이 막으므로 갈 수 없는 링크를 그리지 않는다.
    assert expected not in anonymous.text


def test_Notion_중단화면의_버튼을_그대로_열면_405가_아니다(gate_stopped):
    report = gate_stopped
    job_id = f"gate-notion-405-{uuid.uuid4().hex}"
    cookies, csrf = _admin_cookies()
...
def test_일시장애_안내화면은_조사번호를_화면에_되비추지_않는다(monkeypatch):
    ...
    secret_id = "attacker-controlled-report-id-secret"
    ...
    with TestClient(app) as client:
        response = client.get(f"/download/pdf/{secret_id}", follow_redirects=False)
...
def test_자동검사중단_라우터가_출구_주소와_글자를_직접_넘긴다(gate_stopped):
    ...
    job_id = f"gate-context-{uuid.uuid4().hex}"
    cookies, csrf = _admin_cookies()
