...
def test_누적이_정확히_상한에_닿는_예약까지는_들어간다(날짜를_옮긴다) -> None:
    ...
    확인 21건(2,100원) 뒤 남은 몫은 정확히 900원이다. 본조사 한 건은 들어가고,
    그 결과 누적은 정확히 상한이 된다.
    """
    ...
    assert 표 is not None, "잔여가 정확히 900원인데 본조사가 막혔다"
    assert _안내숫자() == _누적상한
...
def test_상한을_단_1원이라도_넘기면_막힌다(날짜를_옮긴다) -> None:
    ...
    assert _확인을_반복한다(22) == 22
    assert _누적바닥값() == 2200.0

    날짜를_옮긴다(1)

    assert _본조사를_예약한다("day2-over") is None, (
        "2,200 + 900 = 3,100원인데 본조사가 들어갔다"
    )
