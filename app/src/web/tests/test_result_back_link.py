"""초대 링크 손님이 결과 화면에서 «돌아갈 길»과 «언제 자료인지»를 본다 (티켓 G-S7).

★ 무엇이 문제였나 — G-S6이 `/k/`의 도착지를 랜딩으로 바꾼 뒤, 손님이 보고서를
...
★ 그리고 결속 보고서 위에는 「{날짜}에 만든 보고서」 한 줄을 둔다. 안 알리면
  인사팀이 «옛 숫자»를 오늘 숫자로 읽는다 (사람 결정 D-G10·D-G11).
...
def _결속보고서를_저장본으로_연다(
    ...
    ★ 왜 저장본 갈래인가 — legacy 갈래는 `_link_expired`가 참이면 라우트가
      ...
      보고서 신선도보다 긴 실제 상황(사람 결정 D-G8: 링크 90일)에서는
...
def test_결과화면_띠와_버튼은_내부용어를_쓰지않는다(
    ...
    """★ 「우리」는 만든 쪽을 가리켜 받는 사람을 헷갈리게 한다 (D-G10).
...
def _골든(이름: str, 확장: str) -> str:
    """G-S7 수정 «전»(b15bb06) 코드로 찍어 둔 결과 화면.

    ★ 다시 찍는 법 — 결과 화면을 일부러 바꾸는 티켓에서만, 그 커밋의 코드로
