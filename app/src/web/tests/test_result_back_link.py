...
def _골든(이름: str, 확장: str) -> str:
    ...
    ★ 2026-09-17에 다시 찍었다 — 카드 「범위·한계」·「운영 범위·한계」 줄을
      세 채널에서 «뺀» 변경이다. 세 손님 파일 모두 «지워진 글자 줄 10개»
      (라벨 5줄 + 값 5줄)뿐이고 새로 생긴 줄은 0이며, `head.html` 세 개는
      한 바이트도 안 바뀌었다 — 표지 위 장식은 이 변경과 무관하다.
    ★ 2026-09-17(같은 날 두 번째): 라벨이 아니라 «값 안»에 남아 있던 한계 문구도
      뺐다. 세 손님 파일 모두 «바뀐 글자 줄 13개»(3장 라벨·값 각 2줄, 5장 값
      1줄, 6장 라벨·값 각 3줄, 9장 라벨·값 각 1줄)뿐이고 줄이 늘거나 줄지
      않았다. `head.html` 세 개는 이번에도 안 바뀌었다.
    ★ 2026-09-17(같은 날 세 번째): 6장 값의 「효과: 」 접두를 뺐다. 세 손님 파일
      모두 «바뀐 글자 줄 3개»(6장 값 3줄)뿐이고 줄이 늘거나 줄지 않았다.
    ★ 2026-09-22: 세 손님 파일 모두 «빠진 줄 7·생긴 줄 5»로 같다(공개 286→284,
      회원 307→305, 관리자 290→288줄) —
      ① 마스트헤드 생성일이 표지와 같은 점 표기(2026.08.19)가 됐다,
      ② 부록 위에 「이 보고서에는 외부 언론 보도 출처가 없습니다.」 한 줄이 생겼다,
      ③ 부록 자료 열이 「제목」+「· 발행처」 두 줄에서 「제목 · 발행처」 한 줄로 합쳐지고
      회사 공식 웹 출처는 발행처 자리에 「회사 공식 웹」이 온다. `head.html` 세 개는 안 바뀌었다.
    ★ 2026-09-23: 4장 제목 한 줄만 기간 중립 문구로 바꿨다. 정본 제목만 옛 문구로
      임시 복원한 실제 세 독자 화면이 직전 골든 전체와 일치함을 먼저 확인했다.
      `head.html` 세 개와 날짜·숫자·출처 표시는 그대로다.
    """
