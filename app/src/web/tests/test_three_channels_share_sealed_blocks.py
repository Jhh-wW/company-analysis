"""웹·PDF·노션 세 채널이 «같은 봉인 블록»의 같은 글자를 낸다 (적대 검토 F1 ③).

★ 왜 이 파일이 따로 필요한가
  S4(PDF)·S5(웹)·S6(노션) 세 티켓은 각각 자기 채널 하나만 봤다. 채널마다
  「봉인 값을 썼다」는 시험은 있었지만, **같은 저장본 하나를 세 채널에 동시에
  통과시켜 글자를 맞대 본 시험이 없었다.** 그래서 한 채널만 조용히 빠져도
  아무 시험이 깨지지 않았다 — 3개년 «변화 요약» 띠 제목이 웹에서는
  ``aria-label`` 속성에만 있고 눈에 보이는 글자로는 없던 결함(F1 ③ P1-1)이
  그렇게 살아남았다.
...
★ 채널 고유 «모양»은 허용한다(root 결정 D-PUB4). 웹은 인용 번호를 위첨자
...
def _render_from_stored_delivery(
    ...
    ★ legacy snapshot 갈래는 화면을 ``legacy_readonly=True``로 그려 부분 보고서
      고지가 통째로 꺼진다(F-S5d). 고지를 보는 시험이 그 갈래로 그리면
