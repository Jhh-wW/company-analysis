...
def test_오늘_합계는_회원별_값의_합이다(admin: TestClient):
    ...
    assert 화면.status_code == 200
    # 4,500 + 6,000 = 10,500원 · 7 + 3 = 10건
    assert "10,500원" in 화면.text
...
def test_오늘_화면은_기본값을_전원_규칙이_아니라_기본값이라고_말한다(
    ...
    assert (
        "따로 정하지 않은 친구는 하루 6,000원 입장 기준 + 성공 3건입니다"
        in 화면.text
    )
...
def test_접근_화면_합계는_회원별_한도의_합이다():
    """★ 한 명만 올려도 「N명 × 6,000원」은 실제 노출보다 작아진다.
    ...
    assert 화면.status_code == 200
    # 6,000 + 6,000 = 12,000원. 여기에 관리자 100,000원을 더해 112,000원.
    assert "초대한 친구 2명 합계 12,000원" in 화면.text
    assert "112,000원" in 화면.text
    # 한 명이라도 다르면 「N명 × 6,000원」은 거짓이므로 쓰지 않는다.
    assert "명 × 6,000원" not in 화면.text
...
def test_아무도_한도를_안_바꿨으면_접근_화면_문구가_그대로다():
    ...
    `test_admin_access.py`의 기존 회귀시험이 「친구 N명 × 6,000원」과
    ...
    assert "초대한 친구 1명 × 6,000원" in 화면.text
    assert "성공 보고서 3건" in 화면.text
    assert "비용 입장 상한 6,000원" in 초대화면.text
    # 링크 0개 + 친구 6,000 + 관리자 100,000 = 106,000원
    assert "106,000원" in 화면.text
