...
def test_봉인을_주장하는_보고서의_봉인_행이_사라지면_공개_결과가_닫힌다(
    ...
    """★ 뒤집힌 시험이다 — 예전에는 「봉인 없음으로 열린다」였다.

    뒤집은 근거 — 두 규칙은 층이 달라 서로 어긋나지
    않는다.
      · 저장층(``reports.load``)은 봉인 행이 없으면 ``None``을 그대로 돌려준다.
        그건 「봉인 없음」이라는 정의된 상태다(그 시험은 지금도 초록).
...
# ══════════════════════════════════════════════════════════
# ⑤ public_id != report_id — 조용한 「봉인 없음」 금지
# ══════════════════════════════════════════════════════════
...
def test_공개ID가_보고서ID와_다르면_봉인_없음으로_조용히_넘어가지_않는다(
    ...
    말하고 있다. 말과 실제가 다르면 그리지 않고 닫는다.

    발급 경로 두 곳(``routers/reports.py``의 ``persist_reused_delivery``·
    ``persist_approved_delivery`` 호출, 현재 1464·1546행)은 언제나
...
def test_증거가_봉인을_말하지_않는_보고서는_봉인_없이도_열린다(
    ...
    """반대 경우 시험 — 위 규칙이 봉인을 «주장하지 않는» 보고서를 막지 않는다.
