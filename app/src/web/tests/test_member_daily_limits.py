...
★ 반쪽이면 안 되는 이유 — 성공 건수만 회원값을 따르면, 한도를 1건으로 낮춘 친구도
  실패를 반복하며 하루 6,000원을 그대로 쓴다. 비용만 회원값을 따르면, 한도를 5건으로
...
★ 기준값은 리터럴로 적는다 — 생산 상수를 가져와 자기 자신과 비교하면 값이 내려가는
  회귀를 못 잡는다. 지금 계약: 회원 기본 하루 6,000원 · 성공 3건 · 본조사 예약 2,000원
  (2026-09-13 1,000→2,000 인상, 근거는 `budget/constants.py` 주석. 그 전에는
  2026-09-05 1,800→1,000 하향).
"""
...
#: 본조사 한 번이 미리 잡는 예상액. `budget/constants.py`의 계약값과 같은 2,000원을
...
def test_회원별_한도는_성공건수와_비용_둘_다_적용된다():
    """성공 2건에서 3번째가 거절되고, 하루 3,600원이면 본조사 두 번째가 거절된다."""
    _초대한다(_친구)
    _한도를_정한다(_친구, 건수=2, 금액=3600.0)
    ...
    # (2) 비용 — 그 상한이 예약 커밋 자리까지 간다. 3,600원 상한에서 본조사(2,000원)는
    #     한 번만 들어가고, 두 번째는 상한을 넘으므로 거절된다(기본 6,000원이면 통과했을 것).
...
def test_한도를_안_정한_친구는_기존_6000원과_3건_그대로다():
    ...
    # 6,000원이면 2,000원짜리 본조사가 «정산 전 예약 기준»으로 세 번 들어가고
    # 네 번째가 거절된다(2,000×4 > 6,000). 실제 운영은 각 조사가 실제 지출로
...
def test_한도를_올린_친구는_비용도_성공건수도_같이_올라간다():
    _초대한다(_친구)
    _한도를_정한다(_친구, 건수=5, 금액=9000.0)
    오늘 = clock.today_kst().isoformat()

    _track, bucket, cap = request_helpers._track_of(_친구로_들어온_요청(_친구))
    assert cap == 9000.0

    # 9,000원이면 2,000원 본조사가 네 번 들어가고 다섯 번째에서 막힌다.
...
def test_예약액_계약은_본조사_2000원_그대로다():
    """상한 경계 시험이 기대는 값이 바뀌면 위 시험들의 뜻도 바뀐다.

    이전 이름: `test_예약액_계약은_본조사_1000원_그대로다`
    (2026-09-13 1,000→2,000 인상). 그 전 이름은
    `test_예약액_계약은_본조사_1800원_그대로다` (2026-09-05 하향).
    """
...
@pytest.mark.parametrize(
    ("건수", "금액"),
    [("0", "1000"), ("21", "1000"), ("3", "-1"), ("3", "40001"), ("셋", "1000")],
)
def test_범위_밖_값은_400이고_아무것도_저장되지_않는다(
    ...
    (실측: `allowlist.py`의 성공 건수 범위 조건을 끄고도
