"""회원별 하루 한도가 «성공 건수와 비용 둘 다»에 실제로 닿는지 본다 (G-S5, D-G4 (a)).
...
def test_한도를_안_정한_친구는_기존_3000원과_3건_그대로다():
    """★ 음성 대조 — 회원별 한도를 넣었다고 기존 계약이 흔들리면 안 된다."""
    _초대한다(_친구)
    오늘 = clock.today_kst().isoformat()
...
def test_명단_밖_로그인은_회원_한도를_읽지_않고_0원인_손님이다():
    """★ P-95 그대로 — 로그인만 한 사람에게 회원 한도가 붙으면 안 된다."""
...
def test_기본한도_변경은_다음날도_유지된다(admin: TestClient):
    """★ 영구 값이다 — 오늘만 쓰고 사라지는 보너스가 아니다 (D-G4 (a))."""
    _초대한다(_친구)
    admin.post(
        _한도경로,
...
def test_옛_좁은_운영판_계약에서는_한도_변경이_초대와_똑같이_막힌다(
    ...
    """★ 음성 대조 — 친구 초대가 막힌 배포판에서는 한도 변경도 같이 막힌다."""
    _초대한다(_친구)
    monkeypatch.setenv(
