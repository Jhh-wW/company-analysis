"""보고서에서 «빼기로 한 칸»이 정말 안 나가는지 못 박는다 (문제로그 P-111).

★ 2026-08-16 사용자 지시: 「9번 이 칸 없애자」.
...
def test_뺀_칸이_정해져_있다():
    assert "9" in HIDDEN_CELLS


def test_데모_보고서에_9번이_안_나온다():
...
def test_채운_개수는_공고블록과_숨긴9번을_세지_않는다():
    report = Report(
        company="가나다",
        job="영업",
        corp_type="상장사",
