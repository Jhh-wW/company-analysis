"""데모의 «수집 현황»이 사실을 말하는지 못 박는다.

★ 이 시험이 잡는 것 — **기능은 붙었는데 화면이 옛말을 하는 경우**.
  홈페이지 수집은 이미 붙어 있는데(해소, `features/homepage/`), 데모 화면은
  「아직 연결되지 않음」이라고 말하고 있었다. 그러면 사용자는
  **「이 도구는 홈페이지를 못 본다」**고 잘못 안다. 없는 것은 «기능»이 아니라
  이 저장 기록이다 — 초기 조사 엔진이 기록을 만들 때는 수집 기능이 없었을 뿐이다.

★ ❌(없음)로 적어도 안 된다 — 「회사에 홈페이지가 없다」는 뜻이 되어 버린다.
...
def test_데모는_홈페이지가_미연결이라고_말하지_않는다(회사: str):
    """★ 데모는 홈페이지를 「미연결」이라고 말하지 않는다. 「연결」이라는 말로 기능의 부재를 암시하면 안 된다."""
    status = _홈페이지_현황(회사)
...
def test_데모_홈페이지는_없음이_아니라_못_가져옴이다(회사: str):
    """❌(회사에 자료 없음)로 적으면 멀쩡한 회사를 포기하게 만든다."""
    status = _홈페이지_현황(회사)
