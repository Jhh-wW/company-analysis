...
#   도식 검수 1 + 요약 작성 1 + 요약 검수 1 = 3, 재검수 1.
_필수후속 = 3
_재검수몫 = 1
_재작성예약 = _필수후속 + _재검수몫  # 4
_상한 = 18
_뉴스몫 = 5
_장작성 = 9
...
def _재생(*, 재작성예약: int, 재검수예약: int, 뉴스: int) -> tuple[list[str], list[str], int]:
    ...
    (선택) 재작성 ≤3 → (선택) 재검수 → 도식 검수 → 요약 작성 → 요약 검수.
    """
...
def test_필수후속몫은_도식_요약작성_요약검수_3회다() -> None:
    ...
    assert report_recovery.SUMMARY_REVIEW_CALLS == 1
    assert report_recovery.MANDATORY_TAIL_AI_CALLS == _필수후속
    assert report_recovery.REWRITE_RECHECK_CALLS == _재검수몫
    # 뉴스가 남겨야 하는 몫 = 장 작성 9 + 본문 검수 1 + 필수 후속 3.
    assert report_recovery.MANDATORY_REPORT_AI_CALLS == 13
...
def test_예약이_걸린_호출은_경계에서_정확히_한_번_더_거부된다() -> None:
    ...
    for _ in range(_상한 - _필수후속 - 1):  # 14회
        engine.reserve_provider_call()
    # 15번째까지는 «필수 후속 3을 남기고도» 부를 수 있다.
    assert engine.reserve_provider_call(reserved_calls=_필수후속) == 15
    # 16번째부터는 남은 3이 필수 후속 몫이라 선택적 호출은 막힌다.
    ...
    assert engine.reserve_provider_call() == 16
...
def test_예약없는_옛_배선이면_도식요약이_굶는다() -> None:
    ...
    assert 거부됐다 == ["재검수", "도식검수", "요약작성", "요약검수"], (
