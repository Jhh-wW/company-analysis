...
def test_스위치OFF면_3장_부족과_뉴스미조사_안내를_포함한_바이트골든을_유지한다(
    ...
    # 2026-09-11(2): 8장 두 문장이 인용 원문의 낱말을 쓰도록 바뀌어 골든을 다시
    #   만들었다. 8장 원문 절 계약이 «후보가 기댄 절»을 보게 되면서, 인용 원문과
    #   낱말이 하나도 겹치지 않던 두 문장은 어느 절에 기댔는지 가릴 수 없어
    #   빠졌다. 필드별 대조에서 바뀐 칸은 그 두 자리뿐이다(회사 사실·문장 수·
    #   안내문·등급 전부 그대로):
    #     · sections[7].prose_lines[1] 「Leader's Code로 …」
    #       → 「공식 표어와 함께 Leader's Code로 …」
    #     · sections[7].prose_lines[3] 「감각과 경험을 …」
    #       → 「시제품 시험과 피드백 반영 같은 업무 사례는 감각과 경험을 …」
    ...
    # 2026-09-11(1): 골든에서 «단 한 자리»만 바뀌었다 — 세 번째 핵심 요약이
    #   1장 «첫» 문장 축자 복제에서 1장 «둘째» 문장으로 옮겨 갔다.
    # 2026-09-11(2): 요약이 「AI가 새로 쓴다」에서 「검증된 본문 문장 중
    #   고른다」로 바뀌면서 골든을 다시 만들었다. 필드별 대조로 확인한
    #   변화는 «두 가지»뿐이다(전체 208칸 중 17칸, 나머지 191칸 동일):
    #     ① summary_items 3건이 AI가 쓴 문장에서 1·2·3장의 검증된 본문
    #        문장으로 바뀌었다(장도 서로 다르다).
    #     ② shortfall_reasons에서 「핵심 요약에서도 같은 이유로 숫자 문장
