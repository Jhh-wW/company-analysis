...
def test_스위치OFF면_3장_부족_바이트골든을_유지한다(
    ...
    #   바꾸면서 작가 문장을 원문 그대로 되돌렸다. 최종 산출 바이트는 이 줄
    #   위의 2026-09-08 기록과 같다(필드별 대조로 확인).
    ...
    # 2026-09-22: 필드별 대조로 확인한 변화는 «두 가지»뿐이다(206칸 중 16칸, 나머지 190칸 동일):
    #   ① 해석 문장 13개에 자기 인용 번호가 보인다(「— 해석」 앞 [N]) — 해석 문장도
