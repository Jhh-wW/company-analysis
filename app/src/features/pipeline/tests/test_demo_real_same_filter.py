"""데모와 진짜 조사가 «같은 문장 판정»을 쓰는지 못 박는다 (문제로그 P-83).
...
★ 왜 이 부류가 반복되나 — 같은 판단을 **두 곳에** 적어 두면 한 곳만 고쳐진다.
  P-74에서 앞머리 떼기를 «같은 함수»로 합치며 이미 겪었는데, 잘림 검사는 합치지 않았다.
...
#: 데모 기록이 있는 회사들. 4번이 차는 곳·안 차는 곳을 섞어 둔다.
_표본 = (
    "파마리서치", "토스씨엑스", "루트로닉", "카카오", "하이브",
    "넥스트증권", "우리엔", "로보스타", "엠투아이코퍼레이션",
    "인텍에프에이", "카카오모빌리티", "플래티어",
)
...
def test_데모_4번에_못_쓸_문장이_없다():
    """★ P-83 그 자체. 실측 — 걸러내기 전 13문장 중 1개가 잘린 토막이었다."""
    범인 = [
        f"[{회사}] {문장[:70]}"
...
def test_실제로_새어나갔던_그_문장이_지금은_막힌다():
    """★ 「증상이 안 보인다」가 아니라 «그 입력»을 직접 넣어 확인한다 (P-75 교훈)."""
...
def test_걸러내도_4번이_비지_않는다():
    """★ 여기가 깨지면 규칙이 과해 «쓸 만한 문장까지» 죽인 것이다.

    실측 (2026-08-16) — 걸러내기 전후로 **4번이 찬 회사 8곳이 그대로**였고
    문장은 13개 → 12개로 «쓰레기 1개만» 빠졌다.
    """
    찬_회사 = {회사 for 회사, _ in _데모_4번_인용문장()}

    assert len(찬_회사) >= 8, (
        f"4번이 차는 회사가 {len(찬_회사)}곳으로 줄었습니다 — 걸러내기가 과합니다"
    )
