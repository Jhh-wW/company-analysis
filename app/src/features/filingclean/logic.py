...
★ 무엇이 잘못됐나 — 1판은 절 표제를 찾을 때 **첫 출현만** 본다:
  ...
  실측 — 상장 엔터사 사업보고서 조각 **8개 중 3개가 목차**(사업내용·재무·MD&A).
  그래서 1·3·4번 칸이 통째로 비었다.
  저장 기록 전체로는 238개 중 **14개(5.9%)**다 — 로봇 제조사는 **9개 중 5개**가 목차였다.
...
# 다른 엔터사의 「상세 내용은 본문 참조」 요약과 「재무제표 주석을 참조」 교차참조가
...
def repair(
    ...
    for fid, frag in frags.items():
        ...
        #     실제로 상장 엔터사 MD&A가 그랬다: 목차 → (고침) → 면책 문구.
