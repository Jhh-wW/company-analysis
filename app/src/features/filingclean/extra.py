...
★ 왜 필요한가 — 결과가 전자공시 원문을 그대로 옮긴 것과 다를 바 없어 보였다.
  ...
  실측 (상장 엔터사 사업보고서 375,934자, 직접 확인) —
  1판이 뜨는 절 8종에 **아래가 하나도 없다:**

  | 안 뜨던 절 | 실제 내용 |
  |---|---|
  | **신규사업 등의 내용 및 전망** | 「이 회사는 **멀티 레이블 전략을 통해서 성장을 지속시켜나갈 계획**입니다. 한국·일본·미국에 걸쳐 15개의 독립 레이블을 운영」 |
...
EXTRA_SECTION_HEADS: Final[dict[str, tuple[str, ...]]] = {
    ...
    #
    # ⚠️⚠️ **기대를 낮춰야 한다 — 실측 결과 (상장 엔터사 사업보고서)**
    ...
    #
    #   실측 —
    #   · 소재 제조사: 「번호 구분 내용 권리자 **출원일 등록일** 적용제품 출원국 **현재상태**」
    ...
    #   전부 **무형자산 장부금액 표**였다:
    #     대형 제조사  「산업재산권 회원권 기타 … 4,789,366 272,898 11,507,761 …」
    #     대형 플랫폼사 「산업재산권 장부금액 취득원가 11,951,314 상각누계액 (10,468,580) …」
    #   지원동기에 쓸 것이 하나도 없는 숫자 표다. 「지적재산권」 계열 표제만 남긴다.
    #
    # ★ 실재율: **8/20 (40%)** — 절반 이하다. 없는 회사가 더 많으므로
    #   이 재료에 기대는 칸은 «있을 때만 뜨는» 설계여야 한다.
    #   쓸 만하게 뜬 곳: 플랫폼·제약·방산·엔터·바이오·로봇·여행플랫폼·소재 8개 업종
    #   (로봇 제조사는 「총출원 건수 / 총등록 건수 / **미등록 건수** / 미등록사유」까지 준다)
    LEGACY_KIND_INTELLECTUAL_PROPERTY: (
        "주요 지적재산권 보유 현황",
        "지적재산권 보유 현황",
        "지적재산권 보유현황",
    ...
    #   상장 엔터사 사업보고서에서 실제로 나온 것은 이것뿐이었다:
...
#:   실측 — 상장 엔터사 「신규사업 등의 내용 및 전망」이 정확히 그랬다:
#:     앞: 「이 회사는 멀티 레이블 전략을 통해서 성장을 지속시켜나갈 계획입니다…」 ← 4-3의 핵심 재료
