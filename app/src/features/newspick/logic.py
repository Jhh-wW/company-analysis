...
#: 전자공시가 「임존종보 (Rim John Chongbo)」·「최영일(각자 대표이사)」처럼
...
def ceo_name(profile: dict[str, Any]) -> str:
    ...
    ★ 공동대표면 「이재상, 방시혁」처럼 쉼표로 이어져 온다. 검색어에는
      한 사람만 넣는다 — 두 이름을 다 넣으면 둘 다 나온 기사만 걸린다.

    ★ 괄호 설명도 뗀다 (2026-08-27 실측). 대기업·비상장 30곳을 전자공시에
      직접 물어 보니 대표자 이름은 «30곳 전부» 있었지만, 삼성바이오로직스는
      「임존종보 (Rim John Chongbo)」로 와서 **쉼표가 없어 split(",")에 안 걸렸다.**
      그대로 두면 검색어가 「삼성바이오로직스 임존종보 (Rim John Chongbo) 인터뷰」가 되어
