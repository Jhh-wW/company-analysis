...
#: ★ 왜 필요한가 — 위 규칙은 「㈜엔터사**가**」에서 「엔터사가」를 통째로 집는다.
#:   그대로 비교하면 「엔터사가 ≠ 엔터사」라서 **자기 회사가 「다른 회사」로 세어진다.**
...
def count_other_corps(text: str, company: str) -> int:
    ...
    ★ 조사를 떼고 비교한다. 안 떼면 「㈜엔터사가」가 «다른 회사»로 세어져
...
def prefilter(
    ...
    *,
    ...
    ★ **회사 이름을 여기서 안 본다.** 그게 이번 변경의 핵심이다 —
      이름 맞추기로는 「그룹명」 기사가 엔터사 기사인 걸 알 수 없다.
      그 판단은 AI가 한다.
...
def search_terms(
    ...
    ★ **대표자 이름이 없으면 그 검색은 통째로 뺀다.** 빈칸으로 검색하면
      「엔터사 인터뷰」가 되어 엉뚱한 회사 기사가 그대로 들어온다 (실측).
    """
...
def _word_from(text: str, start: int, company: str) -> str:
    ...
    「…제작사 엔터사미디어코프의 현대사…」 에서 **「엔터사미디어코프의」**를 준다.
    """
...
def _strip_suffixes(word: str, company: str) -> str:
    """낱말 뒤에 붙은 조사·꼬리말을 벗긴다 — 「엔터사측은」 → 「엔터사」."""
...
def looks_like_other_company(text: str, company: str) -> bool:
    ...
    ★ 왜 프로그램이 하나 — AI가 지시문으로 막아도 계속 「엔터사미디어코프」(영화
      제작사)를 엔터사 기사로 골랐다 (실측). **이름이 같은지는 «판단»이 아니라
      «확인»이라 코드가 하는 편이 확실하다.**

    ★ **이름이 아예 안 나오면 False다** — 그게 이번 변경의 핵심이다.
      「그룹명 신곡」처럼 브랜드로만 난 기사를 여기서 되레 막으면 안 된다.
      그 판단은 AI가 이미 했다.
    """
    이름 = (company or "").strip()
...
def fallback_title_match(
    ...
    ★ 덜 잡는다(엔터사는 0건). 그래도 **틀린 기사를 넣는 것보다 낫다.**
