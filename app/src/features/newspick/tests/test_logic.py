...
def test_지시문에_회사_단서가_들어간다():
    ...
    프롬프트 = build_prompt(
        "하이브",
        {"induty_code_nm": "음반 및 기타 오디오물 출판업", "ceo_nm": "이재상"},
...
def test_통째로_돌린다():
    ...
    골랐다, 기록 = pick_with_ai(
        ask, company="하이브", profile={"ceo_nm": "이재상"}, candidates=_후보들()
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("전영현, 노태문", "전영현"),                              # 삼성전자
        ("기우성, 서진석", "기우성"),                              # 셀트리온
        ("손경식,윤석환(각자 대표이사)", "손경식"),                  # CJ제일제당 (쉼표 뒤 공백 없음)
        ("정태영, 조창현(각자대표이사)", "정태영"),                  # 현대카드 (띄어쓰기 다름)
        (
            "정의선, 무뇨스 바르셀로 호세 안토니오, 최영일(각자 대표이사)",
            "정의선",
        ),                                                        # 현대자동차 (3명)
        # ★ 쉼표가 «없는데» 괄호가 붙는 경우 — 고치기 전에는 여기서 깨졌다
        ("임존종보 (Rim John Chongbo)", "임존종보"),               # 삼성바이오로직스
        # 첫 사람에 직함 괄호가 붙어도 뗀다
        ("최영일(각자 대표이사), 홍길동", "최영일"),
        # 홑이름·외국식 이름은 그대로 둔다
        ("이재상", "이재상"),                                      # 하이브
        ("로저스 해롤드 린", "로저스 해롤드 린"),                    # 쿠팡
        ("이승건", "이승건"),                                      # 비바리퍼블리카
        # 값이 없으면 빈 문자열
        ("", ""),
    ],
...
def test_대표자_이름이_없으면_그_검색어를_통째로_뺀다():
    ...
    있을때 = search_terms("하이브", {"ceo_nm": "이재상"}, 질의)
    없을때 = search_terms("하이브", {"ceo_nm": ""}, 질의)

    assert [t[0] for t in 있을때] == ["하이브", "하이브 이재상 인터뷰", "하이브 실적"]
...
def test_괄호가_붙은_대표자도_검색어가_깨끗하다():
    ...
    만들어진 = search_terms(
        "삼성바이오로직스", {"ceo_nm": "임존종보 (Rim John Chongbo)"}, 질의
    )

    assert [t[0] for t in 만들어진] == ["삼성바이오로직스 임존종보 인터뷰"]
