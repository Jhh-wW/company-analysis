"""초대 링크 첫 화면(랜딩) 문구를 글자 그대로 못 박는다.
...
★ 판정 기준은 「쓰지 않을 말」 목록과 사용자 지시(「우리」·「내」 금지)다.
"""
...
#: 「관리자 화면에서만 쓴다」고 못 박은 말 + 코드에서만 쓰는 영어.
_금지어 = (
    "LINK",
    "capability",
    "KRW",
...
def test_버튼A는_회사명을_그대로_넣는다():
    """★ 예: 「하이브 보고서 보기」. 「우리 회사」도 「내 회사」도 아니다."""
    assert (
...
def test_남은_한도는_원_단위로_두_가지를_말한다():
    """★ 하루와 전체는 «다른 값»이다. 하나만 보이면 언제 막힐지 모른다."""
    assert (
