"""초대 링크의 «수명 전체 누적 상한»을 못 박는다 (티켓 G-S3).
...
def test_하루_상한은_그대로_리터럴_3000원이다() -> None:
    """★ 누적 상한을 넣느라 하루 상한을 건드리지 않았다는 음성 대조."""
...
def test_끝난_단계의_예약은_누적에_두번_세지_않는다(
    ...
    """★ 음성 대조 — 이미 정산된 단계까지 더하면 실제보다 비싸게 막는다."""
    _링크를_만든다(conn)
    _예약중인_조사를_넣는다(
...
def test_누적이_남아도_하루가_차면_새조사를_막는다() -> None:
    """★ 반대 방향 음성 대조 — 누적을 넣느라 하루 상한을 무력화하지 않았다."""
    import datetime as dt  # noqa: PLC0415
