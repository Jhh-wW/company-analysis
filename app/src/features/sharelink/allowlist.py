...
#: 위쪽 20 — 하루 성공 20건이면 본조사 예상액만 40,000원(20건 × 2,000원)이라
#: 사고 규모가 관리자 몫(100,000원)의 세 배를 넘는다. 그 위는 실수로 눌렀을
#: 가능성이 더 크다. (2026-09-13 본조사 예약액 2배 인상에 맞춰 다시 계산)
...
#: 40,000원 — 그 위는 사람 하나가 링크 여러 개보다 크게 쓰는 값이라 오타로 본다.
#: (2026-09-13 본조사 예약액·하루 한도 2배 인상에 맞춰 20,000원 → 40,000원)
MEMBER_BUDGET_MIN_KRW: Final[float] = 0.0
MEMBER_BUDGET_MAX_KRW: Final[float] = 40_000.0
...
class AllowedUser:
    ...
    ★ `daily_success_limit`·`daily_budget_krw`는 **관리자가 덮어쓴 값**이다.
      ...
      해석하면 같은 정의가 두 곳이 되어 한쪽만 고쳐진다 — 같은 정의가 두 곳이 되는 함정이다.
    """
