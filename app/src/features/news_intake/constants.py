...
# ── 장별 의미 안내 (뉴스 본문 분석 프롬프트) ─────────────────────────────
#: 칸 이름만 주면 모델이 «매출»이라는 단어를 보고 회사 전체의 기간 실적을
#: 2장(수익 모델)에 넣는다(4차 실측: 전사 매출·성장률 보도 2건이 2장을 채웠다).
#: 장마다 «무엇을 소유하고 무엇을 소유하지 않는가»를 한 줄로 알려 준다.
#: 키 집합은 grounded.ELIGIBLE_SECTIONS와 같아야 한다(import 시 검사).
GROUNDED_SECTION_GUIDE: Final[tuple[tuple[str, str], ...]] = (
    ("identity", "회사가 밝힌 사업 정의·법적 범위·소재지. 뉴스는 법인 정체를 대체하지 "
                 "않으니 다른 장에 맞으면 그 장을 고르세요."),
    ("business_model", "과금 단위·가격·수수료·구독, 고객 유형·판매 채널, 전체 매출의 "
                       "지역·채널·부문 구성 비중. 특정 기간의 매출·이익 금액이나 전년 대비 "
                       "증감 같은 기간 실적은 이 장이 아닙니다."),
    ("portfolio", "특정 제품·서비스의 역할·대상 고객·수명 단계와 그 제품 고유 성과"
                  "(출시 뒤 매출·이용자 등)."),
    ("past_changes", "이미 일어난 변화. 회사 전체 기간 매출·손익·성장률 보도는 "
...
#: 기간 실적으로 보고 2장 밖으로 옮길 수 있는 출발 장. 2장은 과금·고객·구성을
#: 소유하고 기간 실적을 소유하지 않는다. 3장은 제품 고유 성과를 소유하므로
#: 제품 주어를 기계로 가릴 수 없는 한 옮기지 않는다(과잉 이동 방지).
ROLE_REROUTE_FROM_SECTIONS: Final[frozenset[str]] = frozenset({"business_model"})
#: 옮겨 갈 4장 보조 산문 칸. 공식 구조화 실적 칸(historical_performance)이
#: 아니다 — 그 칸은 구조화 검증기 주입 전용이라 뉴스에 주지 않는다.
ROLE_PERFORMANCE_TARGET_SECTION: Final[str] = "past_changes"
ROLE_PERFORMANCE_TARGET_SLOT: Final[str] = "past_changes:change_context"
#: 연·반기·분기 또는 직전/이번 사업연도를 가리키는 기간 표지.
ROLE_PERIOD_PATTERN: Final[str] = (
    r"(?:지난해|작년|전년|올해|금년|상반기|하반기|[1-4]\s*분기|(?:19|20)\d{2}\s*년|연간|당기|전기)"
)
#: 재무제표 성과 지표. «실적»만 단독으로 쓰인 보도도 기간 실적이다.
ROLE_FINANCIAL_METRIC_PATTERN: Final[str] = (
    r"(?:매출액?|영업수익|영업이익|영업손실|순이익|순손실|당기순이익|당기순손실|세전이익|실적)"
)
#: 금액·비율·배수 값.
ROLE_AMOUNT_PATTERN: Final[str] = (
    r"\d[\d,]*(?:\.\d+)?\s*(?:조|억|만|천)?\s*원|\d[\d,]*(?:\.\d+)?\s*(?:%|％|퍼센트)|\d+(?:\.\d+)?\s*배"
)
#: 증감·달성 표현.
ROLE_CHANGE_PATTERN: Final[str] = (
    r"(?:전년\s*(?:대비|보다|동기)|증가|감소|늘었|늘어|줄었|줄어|성장|급증|급감"
    r"|흑자\s*전환|적자\s*전환|돌파|기록)"
)
#: 2장이 소유하는 역할의 표지 — 과금 단위·가격·수수료·구독, 구성 비중,
#: 고객·채널. 하나라도 있으면 기간 실적으로 옮기지 않는다(2장에 남긴다).
ROLE_REVENUE_MODEL_PATTERN: Final[str] = (
    r"(?:구독료|이용료|요금|수수료|과금|단가|가격|사용료|라이선스료|로열티"
    r"|(?:건|좌석|명|인|회|개|대|시간|계정|거래)\s*당"
    r"|(?:월|연|1인|인당)\s*\d[\d,]*(?:\.\d+)?\s*(?:만|천)?\s*원"
    r"|비중|차지|구성|비율|점유|고객|채널|유통|판매처|거래처)"
)
#: 목표·전망·예정 — 계획을 과거 실적으로 옮기지 않는다.
ROLE_FORWARD_PATTERN: Final[str] = (
    r"(?:목표|전망|예상|예정|계획|기대|추정|가이던스|내다봤|내다본|할\s*것|될\s*것)"
)
#: 앞 문장에 의존하는 상대 시점·역참조 표지 — 이 패턴이 있는 문장을 떼면
#: 기준 연도(«그해» = 2024)나 대상(«해당 서비스»)이 사라진다.
ROLE_RELATIVE_BACK_REFERENCE_PATTERN: Final[str] = (
    r"(?:그해|같은\s*해|동\s*(?:기간|분기|연도)|전(?:년|분기)\s*(?:대비|동기)|"
    r"해당\s*(?:서비스|제품|사업|부문|분야|사업부|품목)|이\s+(?:서비스|제품|사업))"
)
#: 역할 조정 진단 키(수집 진단 「주장역할조정」).
ROLE_DIAGNOSTIC_REROUTED: Final[str] = "performance_rerouted"
ROLE_DIAGNOSTIC_SPLIT: Final[str] = "performance_split"
