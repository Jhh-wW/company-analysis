...
class AskFatalError(Exception):
    ...
    ★ 예외의 예외 — «호출 «횟수» 상한»만은 다르다 (실측).
      ...
      버려졌다(카드사·은행 실측 — 완성된 9개 장이 통째로 사라지고
...
class FilingMeta:
    ...
    ★ 왜 이 타입이 필요한가 (실측 결함) — 전자공시 절 조각(사업내용·MD&A 등)에는
      ...
      문서»다. 그런데 v2는 그 문서 신원을 render까지 넘기지 않아, 완성차사
...
#:
#: ★ 왜 이 값이 필요한가 (상장 엔터사 실측 P4-3) — 3개년 표가 「가장 큰 연도 열」
...
def _composition_shape(
    ...
    ★ 왜 필요한가 (실측) — `revenuemix`가 만드는 표는 「구분 · 금액 · 비중」
      ...
      없음 · 3~5행」일 때만 100% 누적 막대를 그린다. 그래서 소재 제조사 실측에서
      **표는 붙었는데 도식은 안 그려졌다.**
...
def _composition_projection(
    ...
    # ★ 다개년 구성 «변화» 표는 줄이지 않는다 (상장 엔터사 실측 P4-3).
