...
def test_일반어_칸은_비우되_구체적인_콘텐츠_이용_칸은_남는다():
    """일반어 정본은 파서·검수에 같고, 구체적인 칸 하나가 있으면 행을 보존한다."""
    ...
    assert rows[0].cells == ("사용자의 AI 콘텐츠 이용", "", "")
...
def test_나뉜다로_밝힌_구성에서도_매출원_이름을_뽑는다():
    """★ 「나뉜다」·「나뉩니다」는 「나뉘」를 부분문자열로 갖지 않는다(나·뉜·다).

    한글은 어간과 어미가 한 글자로 합쳐져서, 표지를 어간 하나로 적으면
    활용형이 통째로 빠진다. 빠지면 «누락 관측»이 조용히 0이 된다.
    """

    assert revenue_stream_names(
        ("매출은 제품 매출, 상품 매출, 용역 매출로 나뉜다.",)
    ) == ("제품", "상품", "용역")
    assert revenue_stream_names(
        ("매출은 제품 매출과 용역 매출로 나뉩니다.",)
    ) == ("제품", "용역")


def test_연도는_매출원_이름이_아니다():
    """★ 「2024년 매출과 2025년 매출」은 기간 비교지 구성 설명이 아니다.

    연도를 매출원으로 세면 도식이 아무리 옳아도 «없는 누락»이 잡힌다.
    집계어 목록은 글자를 그대로 맞추는 방식이라 해마다 바뀌는 숫자를 못 담는다.
    """

    assert revenue_stream_names(("2024년 매출과 2025년 매출로 구성된다.",)) == ()
    assert revenue_stream_names(("2024 매출과 2025 매출로 구성된다.",)) == ()
    # 연도가 아닌 숫자 이름은 그대로 살아야 한다 — 숫자 전체를 버리면 안 된다.
    assert revenue_stream_names(("매출은 3D 매출과 용역 매출로 구성된다.",)) == (
        "3D",
        "용역",
    )
