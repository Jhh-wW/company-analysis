...
def test_잘린_절_때문에_소재_낱말이_갈려도_보존된다():
    """★ 지지 절을 «둘»까지 보는 이유 (실측) — 절 경계가 문장을 자르는 자리.

    「공식 표어는 'Think Brilliant, Act Efficient!'이며 Leader's Code로
    진실·성실·겸손·사랑을 제시한다」는 따옴표 «안»의 느낌표에서 잘린다. 그래서
    소재 낱말(표어)은 앞 조각에, 후보가 기댄 내용은 뒤 조각에 남는다. 지지 절을
    하나만 보면 이 정상 문장이 사라진다.
    """
    ...
    후보 = "공식 표어와 함께 Leader's Code로 진실·성실·겸손·사랑을 제시한다."
...
def test_인사_사실_옆의_금액_절은_반대_근거로_세지_않는다():
    ...
    후보 = (
        "회사는 2026년 1월 노사합의를 통하여 희망퇴직을 실시하기로 결정했고 "
        "이로 인해 2026년 1분기에 인식할 해고급여는 181,305백만원이다."
    )
...
def test_도식은_행_전체가_한_후보다():
    """★ 배선 — 칸마다 따로 걸면 한 낱말짜리 칸이 기댈 절을 못 찾아 정상 행이
    통째로 지워진다. 이 계약은 후보 «어휘»를 검사하지 않으므로, 칸을 이어
    붙여도 서로 다른 칸의 표지가 결합하는 사고(cellwise_problem 머리말)가
    나지 않는다. 그래서 «행 하나 = 후보 하나»로 넘긴다.

    실제 운영 진입점(check_diagrams)에서 단정한다 — 함수만 부르면 배선이
    칸별로 되돌아가도 초록불이다.
    """
    ...
    원문 = (
        ...
        "2025년 연결 매출액은 8,219억원입니다."
    )
    # 2칸 「상시 시행」은 «혼자서는» 원문과 겹치는 낱말이 하나도 없다 —
    # 칸마다 따로 걸면 이 칸 때문에 행 전체가 지워진다. 행으로 보면 1칸이
    # 교육훈련 절에 기대므로 살아남는다.
    행 = FlowRow(cells=("교육훈련 과정", "상시 시행", ""), citations=("8",))
    보고서 = ComposedReport(
    ...
    assert 문화장.flow_rows == (행,), f"행이 지워졌다: {사유들}"
