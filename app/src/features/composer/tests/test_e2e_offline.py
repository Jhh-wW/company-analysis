...
def test_이음매_2장_사업_흐름표도_7장과_같은_사슬을_지나_화면까지_간다(
    ...
    responses = copy.deepcopy(_RESPONSES_FIXTURE)
    # ★ 칸은 «인용한 조각 3이 실제로 밝힌 것»만 적는다. 조각 3은
    #   「회사는 음반·음원, 공연, MD·라이선싱, 광고·출연 부문에서 수익을 얻으며 …」로,
    #   «제작» 역할도 «후속·반복» 재발생도 밝힌 적이 없다. 그 두 낱말은 역할·과금
    #   결속 가드가 근거를 요구하는 자리인데 조각 3으로는 증명할 수 없어 행이 통째로
    #   빠졌다. 이 시험이 지키려는 것은 «2장 경로표가 화면까지 가는 사슬»이므로
    #   사슬은 그대로 두고 미증명 낱말만 뺀다. 원래 칸은 역할 결속 회귀시험
    #   (test_role_binding_integration)에서 «제외되는 행»으로 보존한다.
...
def test_도식_검수는_전용_상한을_쓴다() -> None:
    ...
    # 전체 행의 짧은 대조 근거와 수치 결속 배열을 위한 승인된 전용 상한이다.
    assert V2_DIAGRAM_MAX_TOKENS == 4096
