...
★ 무엇을 골랐고 왜인가 (실측)
  ─────────────────────────────────────────────────────────
  ⛔ **아래 한 문단은 나중에 «틀린 것으로 판명»났다. 정정이 그 뒤에 있다.**
  ...
  **5건**(같은 내용의 중복 저장분 제외)뿐이었고, 회사는 **(주)하이브·(주)진영
  ...
  ⛔ **정정 — 바로 위의 「저장본 전체」와 「회사는 둘뿐」은 사실이 아니다.**
  ...
  **삼성전자(주)** · **현대자동차(주)** (실측).
  ...
  (실패 ①의 패치를 얹으면 그 셋에서 유일 수치가 사라진다) 넷 다 남겼다.
    ...
    hive_8d6ae287.json        (주)하이브      53           0 (2026-09-23 전 1)
    hive_1b7620d0.json        (주)하이브      50           0 (2026-09-23 전 1)

  ★ 기준값 갱신 기록 (2026-09-23 4차 통합, 총괄 결정)
    hive_8d6ae287 의 3장 부문 나열 문장은 이제 지우지 않는다(3장 6 → 7, 뺀 수 1 → 0).
    남는 2장 문장은 여섯 부문 «나열»만 담아 이 3장 문장의 46%만 덮고, 3장 문장은
    「각 부문이 포트폴리오 안에서 서로 다른 역할을 한다」는 제품·서비스 설명을 더
    가졌다. 소유 장이 대신 말해 주지 못하는 설명을 지우지 않는 것이 의도한 개선이다.
    대가로 같은 여섯 부문 나열이 2장·3장에 함께 보인다. 사실이 사라지는 쪽이 아니라
    겹쳐 보이는 쪽이다.

  ★ 기준값 갱신 기록 (2026-09-23 4차 중복 삭제 엄밀 증명, 총괄 확정)
    hive_1b7620d0 의 1장 일본·미국 진출 문장도 이제 지우지 않는다(1장 7 → 8, 뺀 수
    1 → 0). 삭제는 지울 문장의 주장절이 소유 문장의 완전 주장절과 어절 그대로
    대응할 때만 한다. 이 1장 문장은 6장에 없는 「글로벌 시장 확대를 추진 중이며」
    절을 가졌고 나머지 절도 어절이 다르다 — 겹침 60%는 같은 사실이 남았다는 증명이
    아니다. 대가로 같은 일본·미국 계획이 1장·6장에 함께 보인다.
    ⚠️ 이로써 저장본 5건 모두 뺀 수가 0이다. 이 시험대는 이제 «과잉 삭제»만 지키고,
    실물에서 중복을 «실제로 지우는» 양성은 보여 주지 못한다. 삭제 양성은
    test_dedupe_claim_clause_proof.py 의 증명된 동일 절 사례가 맡는다(손글 문장이라는
    한계를 여기 적어 둔다).
...
#: 정리 뒤에 남아야 하는 장별 문장 수 — 실측으로 굳힌 «지금 동작».
...
BASELINE: Final[dict[str, tuple[int, dict[str, int], int]]] = {
    ...
    "hive_8d6ae287.json": (
        53,
        # portfolio 7 그대로 (2026-09-23 전에는 7 → 6). 2장 문장이 3장 부문 설명의
        # 46%만 덮어 지우지 않는다 — 머리말 «기준값 갱신 기록» 참조.
        {
            "identity": 6,
            "business_model": 7,
            "portfolio": 7,
    ...
    "hive_1b7620d0.json": (
        50,
        # identity 8 그대로 (2026-09-23 전에는 8 → 7). 6장 문장이 1장 글로벌 확장
        # 문장의 주장절을 통째로 담지 않아 지우지 않는다 — 머리말 «기준값 갱신 기록» 참조.
        {
            "identity": 8,
            "business_model": 9,
            "portfolio": 7,
