...
# 미래 장의 현재·완료 전용 본문을 구분하는 보조 구문이다.
# 기존 양태 검사는 다른 장도 쓰므로 여기에서만 필요한 활용형을 따로 둔다.
# 출시·추진·목표 같은 명사 하나는 미래 근거가 아니다. 계획을 완료한 문장과
# 앞으로 할 계획인 문장을 구분하며, 실제 인용 근거의 검사는 별도로 유지한다.
FUTURE_SECTION_FORWARD_RE: Final[re.Pattern[str]] = re.compile(
    # ① 「…할 계획/예정/방침/목표」 꼴 — 아직 하지 않은 일에 붙는다.
    r"(?:할|하는|한다는|해\s*나갈|나갈|낼|칠|될)\s*(?:계획|예정|방침|목표)"
    # ② 계획·예정·방침이 «서술어»로 선 꼴.
    r"|계획(?:한다|하고\s*있|하며|이다|입니다|이며|으로|은|는|\s*단계)"
    r"|예정(?:한다|하고\s*있|하며|이다|입니다|이며|으로|은|는)"
    r"|방침(?:이다|입니다|이며|으로)"
    # ③ 「목표로 한다/삼는다」 — 목표가 «앞으로의 지향»으로 쓰인 꼴만.
    r"|목표(?:로\s*(?:한다|하고|삼|잡)|이다|입니다|이며)"
    # ④ 전망·기대·예상이 «서술어»로 선 꼴과 「…할 것으로 …」 꼴.
    r"|전망(?:이다|입니다|된다|됩니다|하고\s*있|이며)"
    r"|기대(?:한다|하고\s*있|된다|됩니다|이다)"
    r"|예상(?:된다|됩니다|하고\s*있|이다)"
    r"|것으로\s*(?:전망|기대|예상|보인다|관측)"
    # ⑤ 「…할 것이다」 꼴.
    r"|(?:할|될|낼|볼|갈)\s*(?:것이다|것입니다|것으로)"
    # ⑥ 시점 부사 — 뒤에 무엇이 오든 «앞으로»를 가리킨다.
    r"|향후|앞으로|추후|내년|차년도|중장기적|장기적으로"
)

FUTURE_SECTION_NO_FORWARD_STATEMENT: Final[str] = (
...
#: «시간»만으로 이뤄진 주제. 시간은 행위 주체가 될 수 없으므로 주체 판정에서
#: 건너뛴다 — 건너뛴 자리는 「주어 생략」과 같게 다뤄진다.
#:
#: ★ 왜 필요한가 (2026-09-10 멀티캠퍼스 실측) — 인용 원문의 계획 문장이 전부
#:   「'26년은 AI 교육 체계를 고도화하고 …」로 시작한다. 표면 정규화가 인용부호를
#:   지우면 「26년은」이 남고, 주제 규칙이 그것을 «명시된 주어»로 읽어 이 회사의
#:   계획을 「남의 계획」(`future_plan_subject_mismatch`)으로 거절했다.
#: ⚠️ 낱말 «전체»가 시간 표현일 때만이다. 「26년차 팀은」처럼 시간이 수식어로
#:    붙은 진짜 주어는 그대로 주어로 남는다.
TIME_TOPIC_RE: Final[re.Pattern[str]] = re.compile(
    r"\A(?:\d{1,4}\s*(?:년도|년|분기|반기|개월|월|주|일)"
    r"|올해|금년|내년|명년|작년|재작년|전년|당해|당기|전기|당분기|전분기"
    r"|상반기|하반기|향후|앞으로|추후|현재|지금)\Z"
)
...
#: 의지·약속 선어말어미 「-겠-」. 공시문이 «앞으로 하겠다»를 적는 가장 흔한 꼴이다.
#:
#: ★ 왜 필요한가 (2026-09-10 멀티캠퍼스 실측) — 그 실행이 인용한 사업보고서의
#:   성장 계획 세 문단은 계획을 «전부» 이 어미로만 적었다: 「…확충해 나가겠습니다」
#:   「…확보해 나가겠습니다」 「…확대하겠습니다」 「…발전시키겠습니다」
#:   「…강화하겠습니다」. 양태 목록에 이 어미가 없어 그 문장들은 계획으로 읽히지
#:   않았고, 작성기가 「…하겠다고 밝혔다」로 옮긴 본문은 뒤의 보고 동사만 «완료»로
#:   읽혀 `future_section_no_forward_statement` 로 6장에서 빠졌다(실측 3건).
#: ★ 여기 «한 곳»에만 더한다 — 장 배치 검사(`future_section_prose_problem`)도
#:   계획 주장 찾기(`_prose_plan_claims`)도 이 목록을 지나가므로, 목록을 두 벌로
#:   늘리면 한쪽만 고쳐져 「장에는 남는데 근거는 요구되지 않는」 구멍이 생긴다.
#: ⚠️ 앞 글자가 한글일 때만 본다. 「-겠-」은 어간에 붙는 선어말어미라 홀로 서지
#:    않는다. 완료·진행 표지가 «먼저» 오면 교대 순서대로 그쪽이 이긴다 —
#:    「운영하고 있으며 확대하겠습니다」의 「운영」은 여전히 진행이다.
PROMISSORY_MARKER: Final[str] = r"[가-힣]겠"
_PLAN: Final[str] = (
...
# ★ 같은 문장 안에서 회사 귀속과 계획·전망 표지가 함께 있을 때만 표와 같은
#   미래 근거를 요구한다. 회사 주어뿐 아니라 명시적인 소유격 귀속도 포함한다.
# ⚠️ 아래 어느 것도 발동시키지 않는다 — 산업·시장의 현재 서술(주어가 회사가 아님),
#    회사의 진행·완료 서술(「하고 있다」·「하였다」), 배경 설명. 표 계약은 그대로다.
# ⚠️ 회사 이름·기사·업종 목록을 쓰지 않는다. 귀속은 GENERIC_SUBJECTS 의 일반
#    명사로만 식별한다 — 고유명사를 알아보지 않으므로 모든 회사에 같은 잣대다.
...
#: 「회사의 비용」·「전략의 일환일 수 있다」 같은 배경·가능성 해석과
#: 「협력회사의」 같은 다른 주체는 이 경계에 포함하지 않는다.
PROSE_COMPANY_ATTRIBUTION_RE: Final[re.Pattern[str]] = re.compile(
    r"(?<!" + WORD_CHAR + r")(?:"
    + "|".join(re.escape(subject) for subject in sorted(GENERIC_SUBJECTS))
    + r")의\s+(?:" + WORD_CHAR + r"+\s+)*?"
    r"(?:계획|전략|방침|목표)(?:\s*의\s*일환)?"
    r"(?:이다|입니다|이며)(?!" + WORD_CHAR + r")"
)
...
FUTURE_PLAN_REVIEW_GUIDE: Final[str] = (
    ...
    "\n"
    "이 장의 «본문 문장»도 회사를 주어로 세우거나 「회사의 … 전략의 일환이다」처럼 "
    "계획·전략·방침·목표를 회사에 명시적으로 귀속하고, 「…할 계획·예정·방침·목표」나 "
    "「…할 것으로 기대·전망」처럼 회사의 계획·전망을 명시했다면, 같은 "
    f"'{FUTURE_KEY}' 배열을 «그 주장마다» 똑같은 다섯 칸으로 «반드시» 채워 단다. "
    "확인·해석 등급 모두 같다. 가능성만 제시하거나 회사 귀속이 없는 주어 생략문은 "
    "이 추가 귀속 조건으로 회사 계획이라고 단정하지 않는다. "
