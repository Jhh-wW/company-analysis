...
⚠️ 이 검사는 «배당» 한 사례를 잡는 좁은 검사다. 「독립된 두 사실을 합쳐 원문에 없는
   새 관계를 만든」 다른 꼴(수량 범위·인과·주체·시점)은 잡지 못한다 — 일반 검사는
   별도 설계 과제이며, 그때 이 검사는 그 안의 한 경우로 흡수한다.
"""
...
from src.features.composer.quantified_relation_constants import (
    DIRECT_PAYMENT, DIRECT_RECEIPT, GROUP_COUNT_RE, GROUP_COUNT_REVERSED_RE,
    NON_ACTUAL, QUANTIFIED_DIVIDEND_UNBOUND, RECIPIENT_CLAIMS, SENTENCE_BOUNDARY,
)


def _kind_key(kind: str) -> str:
    """집단 이름의 비교 열쇠 — 띄어쓰기만 지운다. 「연결대상 회사」와 「종속회사」는 다르다."""
...
def quantified_dividend_problem(text: str, sources: Mapping[str, str]) -> str:
    claims = tuple(
        claim
        for clause in SENTENCE_BOUNDARY.split(unicodedata.normalize("NFKC", text))
        for pattern in RECIPIENT_CLAIMS
        for claim in pattern.finditer(clause)
    )
    if not claims:
        return ""
    clauses = [
    ...
    for claim in claims:
        # 이 검사는 실제 수취·지급을 넓혀 쓴 주장만 다룬다. 계획·부정 문장은
        # 기존 시점·양태 검수에 남겨 참인 부정까지 긍정 관계로 바꾸지 않는다.
        if NON_ACTUAL.search(claim["action"]):
            continue
        count = int(claim["count"].replace(",", ""))
        kind = _kind_key(claim["kind"])
        bound = False
        for clause in clauses:
            for pattern in (GROUP_COUNT_RE, GROUP_COUNT_REVERSED_RE):
                for group in pattern.finditer(clause):
                    if (_kind_key(group["kind"]) != kind
                            or int(group["count"].replace(",", "")) != count):
                        continue
