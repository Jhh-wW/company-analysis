...
def test_확인과_해석이_같은_자료를_쓰면_부분_검증이다() -> None:
    ...
    해석 문장에도 `[1]` 이 보인다(2026-09-22 결정). 번호 표시와 무관하게
    그 자료는 해석에도 쓰였으므로 「부분 검증」이 맞다.
    """
    ...
    # 전제 확인 — 2026-09-22 결정으로 해석 문장도 자기 번호를 보인다. 번호가
    # 보이든 숨겨지든, 해석에 쓰인 자료의 라벨은 「부분 검증」이어야 한다.
    본문 = [text for section in rendered.sections for text, _ in section.prose_lines]
    해석문장 = next(text for text in 본문 if "해석된다" in text)
    assert "[1]" in 해석문장, (
        f"해석 문장에 번호가 보이지 않습니다 — render 규칙이 바뀌었다면 이 시험의 "
