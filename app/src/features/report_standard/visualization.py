...
#:
#: ★ 상한을 5에서 7로 올린 이유 (상장 엔터사 실측) — 상장 엔터사 매출은 6개 부문이고
...
#:
#: ★ 왜 필요한가 (목업 실측) — composer.constants.FLOW_HEADERS_BY_SECTION는
...
_CARD_HEADER_SETS: Final[tuple[tuple[str, ...], ...]] = (
    ...
    ("계획", "시점", "공시된 내용"),  # 6장 성장 계획 (composer가 열 순서를 「계획→시점」으로 바꿈 — «시점 칸에 시점이 들어간다» 수정과 함께)
    ("내건 가치", "일하는 원칙", "확인된 사례"),  # 8장 인재상
    ("제품·서비스명", "제품·서비스 범위", "중점 추진 근거", "사업적 역할"),  # 3장 핵심 제품·서비스 (나중에 더함)
)
...
class ChartPoint:
    ...
    #: 이 값을 0선 «아래로» 그려야 하는가.
    #:
    #: ★ 왜 계열이 아니라 «점»마다 두나 (상장 엔터사 실측) — 예전에는 계열 전체가
    #:   음수일 때만 아래로 그렸고, 한 계열에 양수·음수가 «섞이면» 도식을
    #:   아예 안 그렸다. 그런데 상장 엔터사 당기순이익은 +1,834 → -34 → -2,544로
...
def _trend(table: ReportTable) -> TableVisualization | None:
    ...
    for column in range(1, len(table.headers)):
        ...
        #   하필 «흑자→적자 전환»처럼 가장 중요한 경우였다(상장 엔터사 실측).
