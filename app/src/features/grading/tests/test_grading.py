...
# ★ 짧은 표 — 87자·숫자 2개라 「길이+숫자」 규칙을 그대로 통과했다.
...
def test_짧은_표도_머리말로_잡는다(표):
    """길이·숫자 기준을 둘 다 밑돌아도 머리말이 2개 이상이면 표다."""
...
#: 파일럿 파생 보고서를 지우면서 fixture가 21→10개로 줄어 실측 126에서
#: 가장 작은 파일(6문장)을 뺀 값. 기준값 하향이 아니라 fixture 크기 재도출이다.
#:
#: ⚠️ 이 합계 하한이 잡는 것은 «대량 유실»(여러 파일이 한꺼번에 안 읽힘)이다.
#:   파일 1개의 내용 훼손은 아래 «파일마다 1줄» 단정이 잡고,
#:   파일이 통째로 사라진 것은 test_파일럿_보고서는_조각과_실행기록이_있는_것만_남는다가 잡는다.
#:   실측: 가장 작은 재수집-p005.md의 인용 줄만 비우면
#:   합계가 정확히 120이라 이 단정은 «통과»하고, 파일별 단정이 잡는다.
