...
def test_같은_링크_동시_요청_3개는_누적_상한을_넘겨_예약하지_못한다() -> None:
    """★ P1 그 자체. 잔여 1원인데 900원짜리 세 건이 동시에 들어온다.
    ...
    assert [t for t in 표 if t is not None] == [], "잔여 1원에 900원이 들어갔다"
    assert 최종누적 == 2999.0


def test_동시_요청_중_남은_몫에_들어가는_한_건만_예약된다() -> None:
    """★ 반대 방향 음성 대조 — 다 막아 버리면 그냥 고장이다.

    잔여가 정확히 900원이면 «한 건»은 들어가야 하고, 나머지 둘은 막혀야 한다.
    """
...
def test_누적이_넉넉하면_동시_세건이_모두_들어간다() -> None:
    """★ 음성 대조 — 누적 검사가 정상 사용까지 막지는 않는다."""
    _링크와_지난_원가를_둔다(0.0)
    _cutover()
    ...
    def 예약(index: int) -> bool:
        ...
        try:
            ...
            state_machine.begin_phase(
                ...
                run_limit_krw=None,
                bucket_total_limit_krw=_누적상한,
                bucket_prior_cost_krw=2100.0,   # 잔여 정확히 900원
                lease_owner_id=f"worker:{index}",
                lease_expires_at=_LEASE_EXPIRES_AT,
                started_at=_시각,
            )
    ...
    try:
        ...
        with pytest.raises(state_machine.AdmissionLimitExceeded):
            state_machine.begin_phase(
                ...
                run_limit_krw=None,
                bucket_total_limit_krw=_누적상한,
                bucket_prior_cost_krw=1300.0,   # 1300 + 900(어제) + 900 > 3000
