...
RUN_SUMMARY_STEP_NAMES: Final[tuple[str, ...]] = (
    "5b_뉴스_수집",
    "6_수집_공식근거사전검사",
    # 수집미완료가 참인데 부분 보고서로 안 내려간 실행의 이유(출고를 막지 않은
    # 미완료의 비차단 근거·종류·상태·사유별 개수)가 실린다. 닫힌 코드와 개수뿐이다.
    "6_수집_선택경로미완료",
    "6_수집_DART부분보고서전환",
    "6_수집_공식근거생성입력",
    ...
    "v2_조각_typed전달_불가",
    "v2_composer_완료",
    # 이 실행이 실제로 어느 출고 모드로 끝났는지와 FULL 호출 장부 사용 여부(모드
    # 이름·검수 자리별 개수·참거짓뿐). 운영 서버에는 요약 로그만 남으므로, 빠지면
    # «작성 전 강등»과 «장부를 쓰고 난 뒤 강등»을 로그만으로는 가를 수 없다.
    "8_출고모드_적용",
    # FULL 공개 안전 판정이 출고를 막았을 때의 닫힌 유형 코드·장 id·개수만 실린다.
    # 운영 서버에는 이 요약 로그만 남으므로, 빠지면 «무엇이 막았나»가 다시 콘솔
    # WARNING 한 줄로만 남는다.
    "8_공개안전_차단유형",
