...
def test_비중_열이_없는_표에도_비중을_계산해_넣지_않는다(v2_켬: None) -> None:
    """★★ 비중을 «우리가» 계산하지 않는다 (제품 결정) — 이 계약은 그대로다.

    ⚠️ 예전에는 비중 칸을 비울 자리가 없어 이런 표를 «통째로 버렸다». 지금은
      「구분 · 금액」 두 열로 싣되 **비중 열을 만들지 않는다.** 지켜야 할 것은
      「표를 안 낸다」가 아니라 「없는 비중을 지어내지 않는다」였다.
    """
    ...
    assert len(표들) == 1
    assert 표들[0]["headers"] == ["구분", "매출액 (백만원)"]
    assert 표들[0]["rows"] == [
        ["제품가", "6,000"],
        ["제품나", "4,000"],
        ["합계", "10,000"],
    ]
    # 비중이라는 말이 열 이름·칸 어디에도 없어야 한다 — 없는 값을 만들지 않았다.
    assert not any("비중" in 이름 for 이름 in 표들[0]["headers"])
    assert not any("%" in 칸 for 행 in 표들[0]["rows"] for 칸 in 행)
    assert 표들[0]["caption"].endswith("· 비중은 공시에 없어 적지 않았습니다")


def test_비중_열이_있는_표는_금액만_경로가_다시_싣지_않는다(v2_켬: None) -> None:
    """★★ 비중 검산에 떨어진 표를 금액만으로 되살리면 «잘못 잘린 표»가 샌다.

    아래 표는 금액 합은 맞지만 비중 합이 35%다 — 표가 잘못 잘렸다는 뜻이다.
    비중 열이 «있으므로» 금액만 경로는 손대지 않고 그대로 버려야 한다.
    """
    ...
    assert 표들 == []
    assert "ratio_column_present" in 진단["축별_탈락사유"]["product"]
