...
#: 세로형 표에서 단위 표시 바로 뒤에 오는 금액 열 머리말. 이 위치의
#: 「수익(매출액)」을 첫 행 이름으로 읽으면 정규화가 매출 근거를 지운다.
#: 단위와 머리말이 붙은 경우만 분리하고, 앞 문장이나 행 중간은 찾지 않는다.
AMOUNT_ONLY_VERTICAL_HEADER_RE: Final[re.Pattern[str]] = re.compile(
    r"\(\s*단\s*위\s*[:：]\s*(?:"
    + "|".join(re.escape(word) for word in REVENUE_UNIT_WORDS)
    + r")\s*\)\s+(?:수익\(매출액\)|매출액|영업수익)\s+(?=\S)"
)
...
#: 「매출」 글자가 있어도 매출표가 «아닌» 투자·설비투자(CAPEX) 문맥 어휘.
#: ★★ 왜 필요한가 (실측 2026-09-22, 대형 물류사 사업보고서 「나. 투자계획」) —
#:   DART 서식의 투자계획 표는 「구 분 | 기투자액 | 향후투자액 | 투자기대효과」
#:   열을 쓰고, 기대효과 칸에 「매출증대 및 업무효율 증가」라고 적는다. 태그를
#:   지운 평문에서는 그 칸이 다음 행 이름 앞에 붙어 «행 이름에 매출이 들어가고»,
#:   「국내법인·해외법인」이 지역 축 힌트가 되며, 기투자액의 행 합도 합계와 맞아
#:   금액 전용 세로형 경로가 이 표를 「지역별 매출액」으로 내보냈다.
#: ★ 이 말은 매출표 머리말·행 이름에는 나오지 않는다. 표제·열 이름·행 이름
#:   어디에든 있으면 «매출이 아니다»로 본다 — 비중 열이 있든 없든 같다.
#: ⚠️ 닫힌 목록이다. 「투자」 한 글자로 넓히지 않는다 — 증권사 「투자중개」·
#:   「투자은행」처럼 «사업부문 이름»에 투자가 들어가는 진짜 매출표가 있다.
#:   투자 «금액·계획·효과»를 가리키는 말만 넣는다. 영문은 대소문자를 가리지
#:   않고 견준다(``casefold``).
NON_REVENUE_CONTEXT_WORDS: Final[tuple[str, ...]] = (
    "투자액",          # 기투자액·향후투자액·총투자액
    "투자기대효과",
    "투자계획",
    "투자금액",
