...
def test_v1_Notion_블록은_불변이다() -> None:
    ...
    ★ 완성 보고서 기준값을 2026-09-17에 다시 바꿨다. 카드 맨 아래 「범위·한계」·
      「운영 범위·한계」 줄을 웹·PDF·Notion 세 채널에서 «뺀» 변경이다 — 독자에게
      처리 과정·범위 설명을 싣지 않는다는 결정에 따른다. 갱신 전후 블록을
      필드별로 대조했고 «지워진 표 행 5개» 말고는 차이가 없었다: 4장
      「범위·한계」 2행, 7장 「운영 범위·한계」 1행, 8장 「범위·한계」 2행.
      새로 생긴 블록·글자는 0이다(단방향 삭제). 옛 값
      ``70245aae3204…843e76``은 그 변경 직전 값이다.
    ★ 2026-09-17에는 «부분 보고서» 기준값도 함께 바꿨다. 이번에 뺀 줄이 8장
      말고 4·7장에도 있어서(8장을 통째로 뺀 부분 표본에도 3행이 남아 있었다)
      부분 쪽 블록도 같이 줄었다. 대조 결과 지워진 표 행 «3개»(4장 2행·7장
      1행)뿐이고 새로 생긴 것은 0이다. 옛 값 ``d92a443b7a6d…0db19d``는 그
      변경 직전 값이다.
    ★ 2026-09-17(같은 날 두 번째): 라벨이 아니라 «값 안»에 남아 있던 한계 문구도
      뺐다. 완성·부분 두 기준값 다 바꿨고, 대조 결과 «바뀐 표 칸 7개»(3장 2칸·
      5장 1칸·6장 3칸·9장 1칸)뿐이며 블록이 늘거나 줄지는 않았다. 옛 값은
      완성 ``4709db4766c9…4329bd``, 부분 ``50df573ce278…2b4c98``이다.
    ★ 2026-09-17(같은 날 세 번째): 6장 값의 「효과: 」 접두를 뺐다 — 라벨이 이미
      「회사 제시 효과」라 값에서 한 번 더 말하던 자리다. 완성·부분 모두 «바뀐 표
      칸 3개»뿐이고 블록 수는 그대로다. 옛 값은 완성 ``80bf9982cea6…8bea2e01``,
      부분 ``407e47176aee…1107e0ec``이다.
    """
    ...
    # ★ 2026-09-17 — 「범위·한계」 줄 자체를 뺐으므로 그 칸은 «0개»여야 한다.
    #   라벨만 지우고 문장을 다른 칸에 옮겨 붙이는 되돌림까지 막으려고 라벨과
    #   옛 문구를 함께 센다.
    for 사라진_글자 in (
        "범위·한계",
        "운영 범위·한계",
        ...
        # ★ 2026-09-17(두 번째) — 값 «안»에 붙던 한계 조각. 라벨이 아니라 값
        #   가운데·끝에 있어서 라벨만 세면 못 잡는다.
        "중점 추진 근거·현재 확인·한계",
        "회사 제시 효과·한계",
        "판정·비교 한계",
        "해석 한계:",
        " · 한계: ",
        # ★ 2026-09-17(세 번째) — 라벨과 겹치던 6장 값 접두.
        "효과: ",
    ):
