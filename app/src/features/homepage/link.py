"""회사 홈페이지 주소를 «실제로 열리는 모양»으로 만든다.

★ 왜 필요한가 — 사용자가 소재 제조사 홈페이지 링크를 눌렀더니 브라우저가
  통째로 막았다: **「연결이 비공개로 설정되어 있지 않습니다 · NET::ERR_CERT_AUTHORITY_INVALID」**.
  사용자 반응은 「이 회사 맞냐」였다 — **우리가 잘못된 회사를 찾아 준 것처럼 보였다.**

★ 실측으로 확인한 진짜 원인 —
    https://www.<회사도메인>.co.kr  → CERTIFICATE_VERIFY_FAILED (브라우저가 막는다)
    http://www.<회사도메인>.co.kr   → **200 OK · 제목 「회사 영문명」**  ← 그 회사가 맞다
