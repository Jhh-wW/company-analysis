...
#: ★ 2026-09-09: f29f4e90의 승인된 문화 기본 안내 변경을 반영한다. 두 카드의
#:   «전사 공통 공식 기준»을 «인용 자료에 나타난 범위로 한정합니다»로 바꿨다.
#:   이전 직접 비교에서 공통 407단어의 좌표·폰트는 같았고, 과거 문화 함수만
#:   복원하면 이전 고정값 네 개가 정확히 재현됐다(v1-golden-audit 근거).
#: 2026-09-09(같은 날 두 번째): SM PDF 7·3쪽 실측으로 머리말(러닝 헤더)
#:   수정을 검토했다 — 같은 쪽에 장이 둘 이상 실리면(예: v1 데모의 1장
#:   「기업 정체성」 바로 뒤 2장 「사업 구조와 수익 모델」이 한 쪽에 함께
#:   실리는 자리) 이어지는 «순수 이어짐» 쪽이 마지막 장 대신 첫 장 이름을
#:   잘못 물려받는 결함이 있었다(SM PDF 7쪽 실측). 다만 최종 수정은 그
#:   쪽 «자체»의 머리말은 기존대로 첫 장을 그대로 보여주고, 다음 쪽에
#:   물려줄 값만 마지막 장으로 따로 갱신한다(``_BrandedCanvas.showPage()``/
#:   ``_SectionHeading.draw()`` 참고, 근거는
...
#:   뺐다(3장 2칸·5장 1칸·6장 3칸·9장 1칸). 갱신 전후 본문 글자 대조에서 그 7칸
#:   외에는 바뀐 줄이 없고 칸이 늘거나 줄지도 않았다.
#: ★ 2026-09-17(같은 날 세 번째): 6장 값의 「효과: 」 접두를 뺐다(3칸). 그 3줄
#:   말고 본문에서 바뀐 줄은 없다.
#: 2026-09-22: 생성일 점 표기와 부록 조건부 시작을 v1에도 적용했다.
#: 이전 코드를 재현해 옛 바이트 지문을 확인한 뒤 대조했다. 7쪽이 6쪽으로
#: 줄었고, 본문 낱말 변경은 생성일 구분자 한 건뿐이다.
_V1_DEMO_PDF_SHA256 = "e9e8f8aa2d4f83708ccbdde325063985f92094daa3df6d341f2031142bf3a2df"
_V1_DEMO_PDF_LENGTH = 90918
