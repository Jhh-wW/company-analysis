...
#:   변경으로 갱신했다. 낱말이 줄어드는 변경이므로 이 지문이 «일부러» 깨졌다.
#:   갱신 전 코드와 본문 글자를 직접 대조했고 «지운 5줄 말고는 한 줄도
#:   달라지지 않았다»:
#:     · 4장 「범위·한계 확인된 실행 범위로 한정」
#:     · 4장 「범위·한계 완료 사업연도 세 시점의 관찰이며 원인·지속성은 …」
#:     · 7장 「운영 범위·한계 현재 상태: 공식 근거에서 현재 운영 확인 · …」
#:     · 8장 「범위·한계 인용 자료에 나타난 범위로 한정합니다」 2줄
#:   추가된 줄은 0이다(단방향 삭제). 옛 네 고정값은 그 두 함수만 복원하면
#:   그대로 재현된다.
#: ★ 2026-09-17(같은 날 두 번째): 라벨이 아니라 «값 안»에 남아 있던 한계 문구도
#:   뺐다. 카드 7칸만 바뀌었고 새로 생기거나 없어진 칸은 0이다:
#:     · 3장 「중점 추진 근거·현재 확인·한계」 2칸 — 라벨에서 「·한계」, 값에서
#:       「 · 한계: 공식 근거가 확인한 범위로 한정」 제거
#:     · 5장 「초기 신호·남은 문제」 1칸 — 값 가운데 「해석 한계: … · 」 제거
#:     · 6장 「회사 제시 효과·한계」 3칸 — 라벨에서 「·한계」, 값에서 「 · 한계: …」 제거
#:     · 9장 「판정·비교 한계」 1칸 — 라벨 「판정」, 값은 판정 하나만 남김
#:   그 7칸 말고 본문에서 달라진 줄은 없다(글자가 짧아져 그 칸 안에서만 줄바꿈이
#:   다시 접혔다).
#: ★ 2026-09-17(같은 날 세 번째): 6장 값의 「효과: 」 접두를 뺐다 — 라벨이 이미
#:   「회사 제시 효과」라 값에서 한 번 더 말하던 자리다. 대조 결과 그 3칸의
#:   값 앞 네 글자만 빠졌고 다른 줄은 한 글자도 안 바뀌었다.
#: 2026-09-22: 생성일을 점 형식으로 통일했다. 이전 코드의 골든을 재현해
#: 낱말을 대조했으며 2026-08-19 한 개가 2026.08.19로 바뀐 것만 확인했다.
_BODY_WORDS_SHA256 = (
    "3832ac38267ce51231c0454a101fb8bb7b99dbd95941f8994b45761e4e46a08f"
)
...
#: 2026-09-17: 위 「범위·한계」 줄 제거로 함께 갱신했다. 본문 글자 대조에서
#: 삭제된 5줄 외에 바뀐 줄이 없었고 새로 생긴 줄도 없다.
#: 2026-09-17(두 번째): 위 「값 안의 한계 문구」 제거로 함께 갱신했다. 바뀐 칸은
#: 그 7칸뿐이고, 그 칸의 글자가 짧아지면서 칸 안 줄바꿈만 다시 접혔다.
#: 2026-09-17(세 번째): 6장 「효과: 」 접두 제거로 함께 갱신했다. 바뀐 줄은 그
#: 3줄뿐이고 줄 수·쪽 나눔은 그대로다.
#: 2026-09-22: 위 날짜 형식과 부록 조건부 시작으로 갱신했다. 부록이 9장과
#: 같은 쪽에 들어가 7쪽에서 6쪽으로 줄었고, 출처와 본문 낱말은 보존됐다.
_BODY_LAYOUT_SHA256 = (
    "9b7c478983ba5abdc3cd3711cd7c006e58ff45d18bdae031c4ddcb3ddc1e2af4"
)
