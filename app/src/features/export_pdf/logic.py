...
class _CompositionGraphic(Flowable):
    ...
    ★ 범례 «한 줄»도 내용 길이에 맞춰 늘어난다(실측 —
      상장 엔터사 「MD 및 라이선싱 공식 상품(MD), IP 라이선싱 등」처럼 긴
    ...
    def draw(self) -> None:
        ...
        for series_index, series in enumerate(self.visual.series):
            ...
            for point_index, point in enumerate(series.points):
                ...
                #   강조색이 생겼다(상장 엔터사 4장 당기순이익 실측). 적자라는
                ...
                #   (상장 엔터사 5쪽 실측). 0선 위는 음수 점에서 «항상» 비어 있고
...
#: 짙게 채우면 «가장 모르는 칸»이 도표에서 제일 눈에 띈다(비상장 소프트웨어사 2장 실측).
    ...
    def _draw_chevron_row(
        ...
        *,
        ...
        for column, value in enumerate(flow):
            ...
            #   제일 진하게 나온다(비상장 소프트웨어사 3쪽 실측). 옅은 점선 테두리로
...
def _lead_with_heading(
    ...
    ★ 왜 필요한가 — ReportLab의 ``KeepTogether.wrap()``은 split을 강제하려고
      ...
      항상 새 쪽에서 시작해 앞 쪽 3/4가 비던 원인이다(비상장 소프트웨어사 3쪽 실측).
