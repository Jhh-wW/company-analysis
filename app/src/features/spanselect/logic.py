...
(확정/05_생성/2_규칙/01_출력틀.md:20). 비상장 외감은 MD&A가 감사보고서에 **들어갈 자리
자체가 없어** 4축이 뉴스로 채워지는 것이 기본값이다 (02_유형별소스.md:38-46).
...
def is_audit_opinion(sentence: str) -> bool:
    ...
    정본 05_생성/2_규칙/01_출력틀.md:69·79 — 감사인 위험 신호는 1차에 넣지 않고,
...
def number_sentences(
    ...
    """조각을 문장으로 쪼개 번호를 붙인다 (정본 01_문장스팬선택.md [1] — 0원).
