...
def allows_unsealed_v2_output(report: object) -> bool:
    """정상 SHADOW·기존 엄격 모드만 원본 객체로 렌더한다. FULL 봉인 손실은 거절한다."""
    return (
        getattr(report, "schema_version", "") == ENGINE_V2_SCHEMA_VERSION
        and getattr(report, "release_mode", "") in ("", "SHADOW", "ENFORCE_NO_PARTIAL")
        and getattr(report, "public_projection", None) is None
        and not getattr(report, "public_structure_manifest", "")
        and getattr(report, "generation_evidence", None) is None
        and getattr(report, "quality_contract_version", "") != STRICT_QUALITY_CONTRACT_VERSION
    )
...
def _rendered_display_values(report: object, visible_sources) -> Iterator[tuple[str, str]]:
...
def _internal_key_problems(report: object, visible_sources) -> list[str]:
    return [
        f"{where}에 내부 키 '{value.strip()}'가 노출됐습니다"
        for where, value in _rendered_display_values(report, visible_sources)
        if INTERNAL_KEY_SHAPE.fullmatch(str(value or "").strip())
    ]


def _paragraph_projection_problems(report: object) -> list[str]:
...
def _citation_mapping_problems(report: object, source_numbers, citation_number) -> list[str]:
    problems: list[str] = []
    for section in report.sections:
        for index, table in enumerate(section.tables, start=1):
            ...
            for raw_cite in table.source_cites:
                ...
                if parsed in seen:
                    ...
                    continue
                seen.add(parsed)
    body_numbers = _cited_numbers_in_body(report, citation_number)
...
def v2_validation_problems(rendered: object, *, visible_sources, citation_number) -> tuple[str, ...]:
    """4검사 결과를 사유 목록으로 돌려준다. 비어 있으면 통과다."""
    return tuple(
        [
            *_internal_key_problems(rendered, visible_sources),
            *_paragraph_projection_problems(rendered),
            *_citation_mapping_problems(rendered, tuple(source.number for source in visible_sources), citation_number),
            *_summary_problems(rendered),
        ]
    )


def validate_v2(rendered: object, *, visible_sources, citation_number) -> None:
    ...
    """
    problems = v2_validation_problems(rendered, visible_sources=visible_sources, citation_number=citation_number)
    if problems:
