...
| «시험로 731에» | ``731`` | 단독 위치 조사 «에»만 맨 숫자 뒤에서 지운다 |
...
#: 숫자 바로 뒤에 단독으로 붙어도 수량 단위가 아닌 위치 조사. 도·가·과처럼
#: 단위/주소 구획과 겹치는 한 글자 조사는 이 예외에 넣지 않는다.
_BARE_NUMBER_PARTICLES: Final[frozenset[str]] = frozenset({"에"})


def _strip_particle(word: str) -> str:
    """붙은 낱말 끝의 조사 하나를 지운다.

    낱말 전체가 조사면 — 단독 위치 조사 «에» 외에는 한 글자를 단위로 남긴다.
    «5도»·«종로3가»처럼 단위와 겹치는 조사를
    지우면 맨 숫자가 되어 아무 맨 숫자와 맞는다. 두 글자 이상이면 빈 값(«2019부터»).
    """

    if word in _BARE_NUMBER_PARTICLES:
        return ""
