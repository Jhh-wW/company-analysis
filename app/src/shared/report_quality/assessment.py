...
def _source_registry(
    ...
    for source in candidate.sources:
        ...
        if not source_id or not identity:
            ...
            if type(source.counts_toward_document_floor) is not bool:
                problems.append(f"source_id {source_id}의 공식 문서 계수 표식이 bool이 아닙니다")
            if source.source_kind == "news" and source.counts_toward_document_floor:
                problems.append(f"source_id {source_id}의 뉴스 출처가 공식 문서 수에 포함됐습니다")
...
def assess_safety(
    ...
    for fact_id in dict.fromkeys(public_ids):
        ...
        news_problems = reviewed_news_prose_problems(fact, sources)
        if news_problems is not None:
...
def assess_quality(
    ...
    for fact_id in unique_public_ids:
        ...
        for source_id in source_ids:
            ...
            if source is None or source.counts_toward_document_floor is not True:
                continue
