...
#:
#: ★ 2026-08-29 — 예전에는 ID·이름을 `frozenset` 으로 담았다. 집합은 «순서가
...
#: 「같은 행인가」를 가르는 필드.
#:
#: ★ 2026-08-29 — 예전에는 «행 전체»를 JSON 으로 찍어 비교했다. 그래서 표시
...
def _metric_row(
    ...
    #   서로 다른 행이 섞여 「모호」로 떨어진다(2026-08-29 실측: 분기 9).
...
def _no_match(branch: int) -> bool:
    ...
    ★ 2026-08-29 — 이 대조가 실패하면 4장 누적 증감률 claim 이 통째로
