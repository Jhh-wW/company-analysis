...
#: ⚠️ 왜 닫혀 있나 — 열 이름이 「매출액 (백만원)」으로 굳어 있던 동안 대형 제조사
#:   (억원)·소재 제조사 주석(천원) 표가 백만원으로 붙었다. 숫자는 원문 그대로라 맞지만
...
#:   ① 이름 상한 40자 → 80자 — 대형 플랫폼사 둘째 행 이름이 실측 71자다. 40자면
#:      앞부분이 잘린 이름(「…모바일 및 PC 게임」)이 나온다.
#:   ② 음수 표기 ``△``·``▲``·``−``·``-`` 를 금액·비중 앞에 허용 — 대형 제조사
#:      「기타 부문간 내부거래 제거 등 △301,146 △8.9%」.
#:   ③ ``%`` 기호를 선택으로 — 카드사는 열 이름이 「구성비」이고 값에
...
def normalize_revenue_name(raw: str) -> str:
    ...
    name = str(raw)
    # HTML 원문은 태그만 지우면 ``&nbsp;``가 글자 그대로 남는다(실측: 대형 플랫폼사
...
def is_revenue_total_name_v2(value: object) -> bool:
    ...
    ★ 왜 넓히나 — 카드사 매출표의 합계 행 이름이 「영업수익합계」다(실측).
...
def revenue_percent_total_is_complete_v2(values: Iterable[object]) -> bool:
    ...
    대형 제조사 「△8.9%」 같은 «부(-)의 비중»이 실재하므로 v2는 부호를 읽고
...
def _text_axis_by_hint(value: str) -> RevenueAxis | None:
    ...
    ★ 왜 필요한가 — v2는 표를 «모양»으로 찾으므로 머리말이 「제품별 매출액」
      같은 표제로 시작하지 않는다(실측: 대형 제조사는 「(단위 : 억원, %) 부 문
...
def _header_axis_agrees(header: str, axis: RevenueAxis) -> bool:
    ...
    ★ 왜 「같다」가 아니라 「반박하지 않는다」인가 — v2는 표를 모양으로
      찾으므로 머리말이 축을 말하지 않는 표가 있다(실측: 대형 플랫폼사 매출 실적
