...
``APP_DATA_ROOT``를 설정하지 않으면 기존 ``analysis_engine/data``와
``analysis_engine/logs``를 그대로 쓴다. Render에서는 ``/var/data``를 지정한다.
"""
...
PERSISTENT_ENGINE_DIRNAME: Final[str] = "analysis_engine"

ENGINE_ROOT = Path(__file__).resolve().parents[2]
LOCAL_DATA_DIR = ENGINE_ROOT / "data"
LOCAL_LOG_DIR = ENGINE_ROOT / "logs"
...
def runtime_data_dir() -> Path:
    ...
    return root / PERSISTENT_ENGINE_DIRNAME
