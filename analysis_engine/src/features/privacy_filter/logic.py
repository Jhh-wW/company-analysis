"""공고 판별 3층 — 개인정보 검출·삭제 (Presidio + 한국 패턴). 착수 순서 1-b.
  ...
  · 검출은 Microsoft Presidio Analyzer(MIT · 자체 서버 실행 · 데이터 외부 유출 없음)를 쓴다.
    검출 범위를 고정 표식으로 치환하는 부분은 네트워크·암호 기능 없는 로컬 코드다.
  · 기본 상태로는 한글 이름·주민번호·전화를 못 잡는다(실측)
    → 원인 실측: 한국어 모델의 개체명 라벨(PS·LC·OG)이 Presidio 기본 매핑(PERSON…)과
...
@dataclass
class _ReplacementSpan:
    """익명화 출력에만 쓰는, 원문 값을 담지 않는 가변 위치 정보."""
...
def _replacement_spans(text: str, analyzer_results: Iterable[object]) -> list[_ReplacementSpan]:
    """Presidio의 기본 충돌/공백 병합 규칙을 로컬에서 재현한다."""
    spans: list[_ReplacementSpan] = []
    for result in analyzer_results:
        ...
        if not entity_type or not isinstance(start, int) or not isinstance(end, int):
            raise ValueError("개인정보 검출 결과 형식이 올바르지 않습니다")
        if start < 0 or start > end or end > len(text):
            raise ValueError(
                f"개인정보 검출 범위 [{start}:{end}]가 글자 길이 {len(text)} 밖입니다"
            )
        spans.append(_ReplacementSpan(entity_type, start, end, score))

    spans.sort(key=lambda span: (span.start, span.end))

    # 같은 유형의 겹친 결과는 하나의 범위로 합친다. 끝과 시작이 같은
    # 인접 범위는 겹친 것이 아니므로 각각의 표식을 유지한다.
    merged_same_type: list[_ReplacementSpan] = []
    pending = spans.copy()
    for span in spans:
        pending.remove(span)
        was_merged = False
        for other in pending:
            ...
            if other.entity_type == span.entity_type and overlap > 0:
                other.start = min(span.start, other.start)
                other.end = max(span.end, other.end)
                other.score = max(span.score, other.score)
                was_merged = True
                break
        if not was_merged:
            pending.append(span)
            merged_same_type.append(span)

    # 다른 유형끼리 범위가 완전히 같으면 점수가 높은 결과를, 한 범위가
    # 다른 범위 안에 들면 바깥 범위를 남긴다.
    conflict_free: list[_ReplacementSpan] = []
    pending = merged_same_type.copy()
    for span in merged_same_type:
        pending.remove(span)
        has_conflict = any(
            (span.score <= other.score
             if span.start == other.start and span.end == other.end
             else other.start <= span.start and other.end >= span.end)
            for other in pending
        )
        if not has_conflict:
            pending.append(span)
            conflict_free.append(span)

    # 같은 유형 사이에 일반 공백만 있으면 Presidio 기본값과 같이 공백까지
    # 한 번에 지운다. 탭과 줄바꿈은 합치지 않는다.
    whitespace_merged: list[_ReplacementSpan] = []
    previous: Optional[_ReplacementSpan] = None
    for span in conflict_free:
        if (previous is not None
                and previous.entity_type == span.entity_type
                and re.fullmatch(r" +", text[previous.end:span.start])):
            whitespace_merged.remove(previous)
            span.start = previous.start
        whitespace_merged.append(span)
        previous = span
    return whitespace_merged


def _replace_analyzer_spans(text: str, analyzer_results: Iterable[object]) -> str:
    """검출 범위를 고정 표식으로 바꾼다. 네트워크·암호 기능은 사용하지 않는다."""
    output = text
    last_replacement_index = len(text)
    for span in sorted(
            _replacement_spans(text, analyzer_results),
            key=lambda item: (item.start, item.end), reverse=True):
        marker = REPLACEMENT_BY_ENTITY.get(span.entity_type, DEFAULT_REPLACEMENT)
        end = min(span.end, last_replacement_index)
        output = output[:span.start] + marker + output[end:]
        last_replacement_index = span.start
    return output
...
def erase(text: str, run_id: Optional[str] = None) -> EraseResult:
    ...
    cleaned = _replace_analyzer_spans(normalized, analyzer_results)
