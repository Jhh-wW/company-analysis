...
def test_마감일_날짜와_담당자_전화는_조합_아님():
    # 2026-08-14 독립 검증 회귀 — 접수 마감일(출생 불가능 연도)은 생년월일 값이 아니다.
...
def test_유니코드_대시_표기도_값_조합으로_잡는다():
    # 2026-08-14 독립 검증 회귀 — en-dash 표기 값이 2층을 뚫던 구멍
...
def test_공백_낀_항목명도_검출한다():
    # P-09 회귀 (2026-08-14 OCR 실측) — 「필수 사항」·「우대 사항」이 미검출되어
...
def test_짧은_키워드는_공백_허용이_없다():
    # 독립 검증 발견 회귀 (2026-08-14) — 「채용시」·「마감일」에까지 공백을 허용하면
