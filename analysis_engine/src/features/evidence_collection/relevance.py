...
SLOT_KEYWORDS: dict[str, tuple[str, ...]] = {
    ...
    # ★ 「제품」 계열 표현만 있으면 은행·카드 같은 금융업 공시에서 이 장이
    #   통째로 빈다(실측 — 우리은행 2025 사업보고서에서 product_role·
    #   revenue_link 모두 0건이라 9장 게이트가 portfolio에서 막혔다). 금융업은
    #   같은 사실을 「상품」·「영업부문」·「이자이익」으로 쓴다. 아래 금융업
    #   표현은 그 원문(접수번호 20260313000815)에서 «직접 본 것»만 넣었고,
    #   있을 법한 어휘를 짐작해 넣지 않았다. 「여신」·「수신」처럼 위험관리·
    #   약관 문단 어디에나 나오는 한 단어는 다른 장을 납치하므로 뺐다.
    "portfolio:product_role": (
        "주력 제품", "핵심 제품", "대표 제품",
        # 우리은행 원문 「다. 주요 상품 및 서비스의 내용」·「4-1. 주요 상품 및
        # 서비스 - 기업상품(상세)」 — 제조업 「주요 제품」에 대응하는 금융업 표기.
        "주요 상품",
        # 우리은행 원문 「기업금융·자산관리·글로벌 금융 등 핵심 사업 부문을
        # 균형 있게 성장시켜」 — 부문이 무슨 역할을 맡는지 말하는 표현.
        "핵심 사업 부문",
    ),
    ...
    "portfolio:revenue_link": (
        "매출 기여", "매출 비중",
        # 우리은행 원문 「부문별 영업실적은 이자부문 7조 8,165억원, 수수료부문
        # 7,807억원, 신탁부문 2,123억원입니다」 — 「매출 비중」의 금융업 대응.
        "부문별 영업실적",
        # 우리은행 원문 「양호한 이자이익을 기록했습니다」·「순이자이익은 7조
        # 817십억원」 — 은행 수익의 부문 기여를 말하는 계정 표현. 「순이자이익」·
        # 「비이자이익」도 이 한 표현에 함께 걸리므로 따로 선언하지 않는다
        # (겹쳐 선언하면 한 문장이 신호 2개로 부풀어 장 판정이 왜곡된다).
        "이자이익",
    ),
    ...
    # 금융업 공시는 「핵심가치」·「행동강령」이라는 말을 안 쓰는 경우가 있다
    # (우리은행 2025 사업보고서에는 두 표현 모두 0회 — 그래서 이 칸이 비어
    # culture 장이 막혔다). 같은 뜻을 「윤리강령」·「조직문화」로 쓴다.
    # 「내부통제」·「준법」 한 단어는 위험관리 문단 44건을 함께 끌고 와
    # current_challenges를 납치하므로 넣지 않았다(실측).
    "culture:work_principle": (
        "핵심가치", "원칙에 따라", "행동강령",
        # 우리은행 원문 「임직원 윤리강령의 제정 및 세부실천방안 마련에 관한 사항」
        "윤리강령",
        # 우리은행 원문 「임직원의 직업윤리와 준법정신을 중시하는 조직문화의
        # 정착방안 마련」·「윤리·준법 조직문화 정착방안(案)」
        "조직문화",
    ),
...
SECTION_HEADING_HINTS: dict[str, tuple[str, ...]] = {
    ...
    # 「상품 및 서비스」는 금융업 공시가 제조업 「제품 및 서비스」 자리에 쓰는
    # 같은 표제다(우리은행 원문 「다. 주요 상품 및 서비스의 내용」). 표제 힌트는
