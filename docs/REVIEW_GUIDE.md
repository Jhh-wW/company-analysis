...
- 9장: 자사와 비교사의 공식 자료에서 고객·제품/서비스·시장·기간·정의·회계 범위가
  모두 같은 축만 비교한다. 같은 조건의 양사 수치가 있으면 비교표로 보여 주되, 한쪽
  자료만 있거나 조건이 다르면 숫자를 맞대거나 우위를 추측하지 않는다. 출시 모드는
  유효한 비교가 끝내 없으면 9장을 빼고 내보내지 않고 보고서 전체를 무차감 중단한다.
...
필수 1~9장이나 검사 하나라도 실패하거나 검사 뒤 hash가 바뀌면 부분 화면도 공개하지 않고
전체를 `GATE_STOPPED`한다. 얇은 장이 최대 2개면 그 장만 한 번 보충하고 결과를 다시
검사하며, 보충 뒤에도 미달이면 `STOP_NO_CHARGE`로 끝내 보고서를 내지 않고 비용도
청구하지 않는다. 수동 승인 GET/POST는 `410 Gone`이고 구형 세 역할 승인 테이블은
감사자료로만 남는다.
...
- [ ] 9장이 얇을 때 보충 1회 뒤에도 미달이면 보고서 미출고·무차감으로 끝나는지 확인
