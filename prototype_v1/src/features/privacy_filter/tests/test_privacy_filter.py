...
카나리아 정본: 확정/기획서/01_식별/02_성공기준.md §재는 법
...
from features.privacy_filter.logic import detect, erase
...
# ── 표기 변형 (우회 방지 — 2026-08-14 독립 검증 발견분 회귀 포함) ──────────────
...
def test_라벨_이름_검출():
    # NER 단독은 「성명 홍길동」의 홍길동을 놓친다(2026-08-14 실측) — 라벨 패턴이 잡는다
...
# ── 공고 보존 (과삭제 방지 — 2026-08-14 독립 검증 발견분 회귀 포함) ──────────
...
def test_담당자_전화는_공고여도_지운다():
    # 3층은 「값」 자체를 지운다 — 공고 통과 여부(2층)와 별개 (정본 §3층)
